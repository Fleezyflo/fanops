...
name: fanops-lander
description: >-
  Land-only sub-agent, used ONLY when fanops-orchestrator runs readonly and cannot run git itself.
  Runs exactly the `gh pr merge` command it is handed. Writes no code, fixes nothing.
model: inherit
readonly: false
is_background: false
---
