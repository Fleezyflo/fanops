...
def _hook_spec(max_words: int = 6) -> str:
    ...
    return (
        ...
        f"job is RETENTION — stop the scroll and open a curiosity loop THIS clip pays off.\n"
        f"    THE ONE TEST (apply to every candidate): does the line speak TO THE VIEWER about what THEY "
        f"get by staying, or does it just NARRATE what happens to the artist? Viewer wins; narration "
        f"loses. 'he stopped answering for a reason', 'the promise he made himself', 'front row last "
        f"song', 'she ran a minute he made it' are all THIRD-PERSON SCENE-NARRATION — they recount the "
        f"clip to no one and are BANNED. The frame is the scroller's attention, NEVER a recap.\n"
        f"    Run this PROCESS in order:\n"
        f"    1) PEAK: find the single most arresting beat (punchline, turn, flex, confession, stakes); "
        f"ignore the rest.\n"
        f"    2) FRAME IT AT THE VIEWER (the core rule): DEFAULT to SECOND PERSON — address 'you', the "
        f"person scrolling ('you', 'POV: you ...', 'wait for ...', 'you're not ready for ...'). A "
        f"THIRD-PERSON line ('he ...', 'she ...', 'the ...') is allowed ONLY when it opens a loop the "
        f"viewer urgently wants closed (real stakes/mystery they'll stay to resolve); if it merely "
        f"recounts the scene it FAILS the one test above. Take the SAME beat and aim it at the viewer: "
        f"narration 'he went silent after this' -> viewer 'you'll get why he went silent'; narration "
        f"'the promise he made himself' -> viewer 'wait for what he swore to himself'.\n"
        f"    3) ANCHOR, don't summarize: build the hook AROUND this clip's concrete specific (the line, "
        f"a name, a number, the image, the exact feeling it lands) so the loop is true and the clip pays "
        f"it off. Emotion is welcome (heartbreak, betrayal, an underdog vs the machine, wanting someone "
        f"back) WHEN tied to that specific beat and to the viewer; a bare mood that floats free is not.\n"
        f"    4) PROMISE, DON'T SPOIL: tease the payoff, do not give it away. It is NOT a caption of the "
        f"audio and NOT a quote of the whole line dumped on screen (they can hear it), and NOT praise of the artist "
        f"(do not hype him, rate him, or call a bar his best). It is about the VIEWER, never about the "
        f"artist.\n"
        f"    5) PATTERN-FIT: pick the ONE proven pattern that fits this clip's real energy (do not "
        f"default to wait-for-it). The lines below are SHAPES — fill the {{slot}} from THIS clip's "
        f"specifics; NEVER output a template verbatim (a slot left unfilled is slop):\n"
        f"      * OPEN LOOP / payoff tease (key: open_loop): 'wait for the {{payoff}}'.\n"
        f"      * CURIOSITY GAP (key: curiosity): 'you're not ready for {{x}}', 'the {{thing}} you missed'.\n"
        f"      * COMMENT / OPINION bait (key: comment_bait): 'is this his {{superlative}}?', 'rate this {{x}}'.\n"
        f"      * CONTRARIAN / bold claim (key: contrarian): 'everyone slept on {{x}}'.\n"
        f"      * POV / relatable (key: pov): 'POV: you {{viewer scenario}}'.\n"
        f"      * PROOF / stakes (key: proof): '{{constraint}}, {{result}}' (e.g. 'one take, no autotune').\n"
        f"    6) PORTABILITY TEST (the bar): could this exact hook sit on ANOTHER clip, or is it just the "
        f"artist + a mood? If yes it is generic; reject it and re-anchor to what only THIS clip has — fix "
        f"a confusing line by ADDING the concrete detail (who, what), never by retreating to something "
        f"vaguer. Then TIGHTEN or go CLEAN: <={max_words} words; the clip's own language; no em-dashes, "
        ...
        f"that fits any clip; THIRD-PERSON scene-narration that just recaps; hooking on the EDITING or "
        f"camera ('watch how he cuts', 'drone up, crowd in'); and bait the clip never pays off. A clip "
        f"with no honest VIEWER-FACING hook is better CLEAN (hook = null) than slop.\n")
