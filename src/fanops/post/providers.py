...
These factories return the exact poster classes / uploader functions the hand-written dispatch
returned, and the lazy imports are preserved (building the registry imports no optional dep).
`kind`: 'hosted' = the scheduler holds the social OAuth, FanOps sends a key + remote id
(Postiz/Zernio); 'local' = dryrun; 'direct' (future) = FanOps holds the OAuth + does the native
...
class Provider:
    ...
    def has_creds(self, cfg: Config) -> bool:
        ...
        # to the one cfg home so the postiz/zernio key checks stay single-sourced.
