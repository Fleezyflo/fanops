...
def refresh_store(led: Ledger, cfg: Config) -> dict:
    ...
    reconciles). On PASS, own-reach tags rank first, then the frozen seed merges in so a strong tag we
    have not yet posted still appears in the menu. Returns a summary dict (never raises on a clean run)."""
    ...
    merged = list(own); seen = set(own)
    for t in vetted_menu():                          # frozen seed: keep never-posted reach tags in the menu
        if t not in seen:
            seen.add(t); merged.append(t)
    cfg.hashtags_path.parent.mkdir(parents=True, exist_ok=True)
    cfg.hashtags_path.write_text(json.dumps({"tags": merged}, indent=2))
    return {"written": True, "verdict": "PASS", "own_ranked": len(own), "total": len(merged)}


def cmd_hashtags_refresh(cfg: Config) -> int:
    """`fanops hashtags refresh` — recompute the own-reach store from analyzed posts (doctor-gated).
    Read-only of the ledger; writes ONLY 00_control/hashtags.json. Always exits 0."""
    led = Ledger.load(cfg)
    r = refresh_store(led, cfg)
    if r.get("written"):
        print(f"hashtags store refreshed: {r['own_ranked']} own-reach tags + frozen seed = {r['total']} tags (00_control/hashtags.json)")
    else:
