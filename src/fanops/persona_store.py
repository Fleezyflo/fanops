...
def _persona_dict(p) -> dict:
    ...
    return {"id": p.id, "name": p.name or "", "voice": p.voice or "",
            ...
            "selection_scope": p.selection_scope, "hook_angle": p.hook_angle, "intensity": p.intensity}
...
def baked_personas() -> list:
    ...
    for x in raw.get("personas", []):
        ...
        # MOL-523: content_focus is now free text; cut_policy is the tokens.
        focus = _norm_directive(x.get("content_focus"), "content_focus")
        policy = _norm_policy(x.get("cut_policy"))
        scope_v = _norm_directive(x.get("selection_scope", ""), "selection_scope")
        angle_v = _norm_directive(x.get("hook_angle", ""), "hook_angle")
        ...
        out.append(Persona(id=str(x["id"]), name=str(x.get("name") or ""), voice=str(x.get("voice") or ""),
                           content_focus=focus, cut_policy=policy,
...
def _norm_policy(cut_policy) -> list[str]:
    """Normalize + validate cut_policy (the multi-select moment-kind lever): lowercase, deduped, each in
    CUT_POLICY. A None/non-list -> []. An unknown kind raises (mirrors the enum levers)."""
    seq = cut_policy if isinstance(cut_policy, (list, tuple)) else []
    out: list[str] = []; seen: set[str] = set()
    for c in seq:
        s = str(c).strip().lower()
        if not s or s in seen: continue
        if s not in CUT_POLICY:
            raise ValueError(f"unknown cut_policy: {s!r}")
...
def add_persona(cfg: Config, name: str, voice: str = "",
                id: str = "", *, content_focus: str = "", cut_policy=None,
    ...
    cut LENGTH + FRAMING derive from cut_policy (MOL-523).)"""
    ...
    # MOL-523: content_focus is now free text; cut_policy is the tokens.
    focus_v = _norm_directive(content_focus, "content_focus")
    policy_v = _norm_policy(cut_policy)
    # MOL-521: selection_scope and hook_angle are now free text.
    scope_v = _norm_directive(selection_scope, "selection_scope")
    angle_v = _norm_directive(hook_angle, "hook_angle")
    ...
    with _personas_txn(cfg):
        ...
        plist.append({"id": pid, "name": nm, "voice": str(voice or ""),
                      ...
                      "content_focus": focus_v, "cut_policy": policy_v,
...
def update_persona(cfg: Config, pid: str, *, name=_UNSET, voice=_UNSET,
                   content_focus=_UNSET, cut_policy=_UNSET,
                   selection_scope=_UNSET, hook_angle=_UNSET, intensity=_UNSET, niche=_UNSET) -> str:
    ...
    # MOL-523: content_focus is now free text; cut_policy is the tokens.
    _focus = _norm_directive(content_focus, "content_focus") if content_focus is not _UNSET else _UNSET
    _policy = _norm_policy(cut_policy) if cut_policy is not _UNSET else _UNSET
    # MOL-521: selection_scope and hook_angle are now free text.
    ...
    with _personas_txn(cfg):
        ...
        for d in plist:
            if isinstance(d, dict) and d.get("id") == pid:
                ...
                if _policy is not _UNSET: d["cut_policy"] = _policy
