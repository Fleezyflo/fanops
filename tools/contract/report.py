...
def payload(decision: Decision, *, contract_id: str, digest: str, traits, risk_tier: str,
    ...
    return {
        ...
        "gates": {"content_approval": gates.content_approval,
                  "merge_authorization": gates.merge_authorization,
                  "authorized_parent": gates.approved_head,
...
def render(decision: Decision, *, contract_id: str, digest: str, traits, risk_tier: str, gates,
    ...
    L = [f"## {icon} `{decision.outcome}`  —  rule `{decision.rule}`  ({decision.phase})", "",
         ...
         f"- gates: content `{gates.content_approval}` · merge-authorization "
         f"`{gates.merge_authorization}` · acceptance `{gates.acceptance}`", ""]

    # WHAT the operator authorized is disclosed at the top of the report, not buried in `detail`. A
    # satisfied authorization gate should let the reader see the exact parent it rests on without
    # going looking — a reader who has to go looking has not been told.
    if gates.merge_authorization == "satisfied" and gates.approved_head:
        L += [f"> **Merge authorized by the operator** at parent `{gates.approved_head[:12]}…`, "
    ...
    L += ["<sub>Generated by `python -m tools.contract`. Read-only: this tool writes nothing, adds "
          "no CI job, and enforces nothing — it reports, and a human decides.</sub>"]
    return "\n".join(L)
...
def render_template() -> str:
    ...
    DECLARATION-ONLY (ADR-0106). There is no `## Lifecycle` section and no `created` row: a contract
    is what was authorized, not a diary of what happened to it. `approved_digest` and
    `approval_token` are written ONCE, after the operator answers, and eliding the `approved_digest:`
    line from `D` (`parse.digest_range`) is what makes writing them leave `D` unchanged.
    """
    return """---
...
stop_conditions: []
supersedes: []
approved_digest: <`python -m tools.contract digest <path>` — write this ONLY once the operator has approved it>
approval_token: <the operator's own approval words, verbatim>
---
