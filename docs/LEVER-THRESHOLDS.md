...
| `_MAX_OVERLAP_FRAC` | 0.5 | two picks overlapping more than this fraction of the shorter span are deduped | fewer near-duplicate picks survive | more overlap allowed | `fanops/moments.py:132` |
| `_MAX_TARGET_PICKS` | 30 | per-persona pick ceiling in the moment prompt | model may aim for more picks on long sources | fewer picks requested | `fanops/prompts.py:56` |
| `_target_pick_count` | `round(duration / band.span)` capped 1..30 | how many clips to aim for by source length | more picks on long sources | fewer picks | `fanops/prompts.py:60` |
...
| `_MIN_MOMENT_S` | 0.5s | minimum pick duration (noise floor) | reject shorter spans | allow shorter picks | `fanops/moments.py:129` |
| `_EOF_TOLERANCE_S` | 0.5s | pick may extend past probed EOF by this much | more EOF overrun tolerated | stricter EOF bound | `fanops/moments.py:127` |
