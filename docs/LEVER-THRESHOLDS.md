...
| `_MAX_TARGET_PICKS` | 30 | per-persona pick ceiling in the moment prompt | model may aim for more picks on long sources | fewer picks requested | `fanops/prompts.py:52` |
| `_target_pick_count` | `round(duration / band.span)` capped 1..30 | how many clips to aim for by source length | more picks on long sources | fewer picks | `fanops/prompts.py:56` |
