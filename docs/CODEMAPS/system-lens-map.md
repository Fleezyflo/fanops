...
| 3b | Ingest picks | `moments.ingest_moments` (`moments.py:301`) | agent response | `Moment` born `picked` (owner-stamped `affinities`), source→`picks_decided`; `[]` → non-terminal `moments_empty`; all-invalid → `error` | none | Fail-visible: empty is LOUD but non-terminal, preserves prior moments; discards stale `moment_hooks` gates only (`moments.py:365` — no `moment_casting` gate, removed P11) |
