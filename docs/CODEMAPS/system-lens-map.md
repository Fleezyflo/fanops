...
| 9 | Publish | `post.run.publish_due` → `_publish_one` (`run.py:408,222`); pipeline `_publish_safe` (`pipeline.py:274`) | queued+due posts | claim→`submitting`→`published`/`needs_reconcile`/`failed`; `published_at`+`publish_hour`/`dow` stamped (`run.py:280-285`); `06_published/<day>/<pid>.json` archive (`run.py:25`) | media upload + `poster.publish` (out of lock); Postiz throttle `postiz_publish_per_min` (`run.py:95`) | `AuthError` halts the run (`run.py:277-279`); other errors → per-post `failed` (re-queueable) except `needs_reconcile` (not downgraded, `run.py:280`) |
...
|-------|-------------------------------|------------------------|
| `voice` | pick/hook/caption prompt per-owner slot | `_base_voice` (`persona_directives.py:56`) → `hook_directive` (`:82`), `caption_directive` (`:107`) → hook `personas[].persona` (`moments.py:374-392`), caption `surfaces[].persona` (`caption.py:209,226`) |
| `content_focus` | pick lens + DERIVED cut LENGTH | `_FOCUS_CLAUSE` in per-persona pick payload (`moments._pick_personas`); `_FOCUS_PROFILE` → `derive_cut_spec` (`:41`) → `resolved_cut_spec` → `acc.clip_profile` → `cfg.resolve_clip_profile(acct)` (`config.py:433`) → `render_account_cut` band (`clip.py:706,723`) |
| `energy` | pick/hook bias + DERIVED framing | `_ENERGY_CLAUSE` in pick/hook prompts; `_ENERGY_FRAMING` → `derive_cut_spec` framing (`:42`) → `acc.framing` → `cfg.resolve_top_bias(acct)` (`config.py:443`) → `top_bias` in `render_account_cut`/`reframe_filter` (`clip.py:310-311`), stamped on `Post.top_bias` at mint (`crosspost.py:294`) |
...
(`crosspost.py:228-232`). **Every Post is BORN `PostState.awaiting_approval`** (`crosspost.py:232`, model default
`models.py:220`) with `submission_id="fanops_<hash>"` (client idempotency token, `crosspost.py:240`),
