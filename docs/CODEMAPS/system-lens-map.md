...
| 3 | Request moments (pick) | `moments.request_moments` (`moments.py:261`) | transcript (char-budget bounded `moments.py:194`), peaks, per-persona pick lenses (`_pick_personas` `:220`), 6 survey frames | opens `moments` agent gate, state→`moments_requested` | `keyframes.extract_keyframes` (fail-open `[]`) | LLM gate; per-source quarantine in pipeline (`pipeline.py:115-126`) |
...
| 6 | Captions | `caption.request_captions`/`ingest_captions` (`caption.py:188,276`); pipeline (`pipeline.py:172,231`) | clip, owner×platform scoped surfaces, corpus, content tags | `Clip.meta_captions[surface]`, state→`captioned` (or `held`) (`caption.py:356`) | none (LLM gate) | HOLD on brand-risk/language-mismatch (`caption.py:349-353`); SEED-TAG FALLBACK on missing surface, NOT a hold (`caption.py:342-348`) |
| 7 | Crosspost | `crosspost.crosspost_clips` → `_mint_surface_post` (`crosspost.py:250,155`); pipeline (`pipeline.py:242`) | captioned clips, surfaces, `Moment.affinities` + `affinity_admits`, batch target | `Post` born `awaiting_approval` (`crosspost.py:232`), clip state→`queued` | none | Wrapped so a raise doesn't cost the pass; a FATAL `AuthError` deliberately escapes (`pipeline.py:249-255`) |
...
| 27 | `FANOPS_ACCOUNT_CASTING` | config.py:599 | ON | Gates `casting.affinity_admits` (owner in `Moment.affinities`; `[]` fans all) |
