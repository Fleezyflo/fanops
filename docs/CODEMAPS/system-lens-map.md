...
Downstream, `Accounts.validate` (`accounts.py:218-244`) surfaces config problems (missing ids, routing drift,
malformed rows); `pipeline.advance` logs persona/differentiation strings containing `"persona"` or
`"shared-cut"` as `differentiation_warn` (`pipeline.py:383-385`). So a link that fails to resolve is not itself
flagged, but its downstream effect (no differentiation) may surface at validate time. `delete_persona` deliberately
leaves accounts with a dangling id that falls open (`persona_store.py` docstring).

### 3.4 Downstream effects — every consumer of every field

| Field | Lands at (payload/render key) | Full chain (file:line) |
|-------|-------------------------------|------------------------|
| `voice` | pick/hook/caption prompt per-owner slot | `_base_voice` (`persona_directives.py:56`) → `casting_directive` (`:68`), `hook_directive` (`:82`), `caption_directive` (`:107`) → pick lenses (`moments._pick_personas`), owner-only hook gate (`moments._hook_personas_for_moment` `:384`), caption `surfaces[].persona` (`caption.py:209,226`) |
| `content_focus` | moment-pick lens + DERIVED cut LENGTH | `_FOCUS_CLAUSE` → `casting_directive` (`persona_directives.py:75-76`); `_FOCUS_PROFILE` → `derive_cut_spec` (`:41`) → `resolved_cut_spec` → `acc.clip_profile` → `cfg.resolve_clip_profile(acct)` → `crosspost.account_render_spec` (`crosspost.py:86-91`) → `render_account_cut` band (`clip.py:706,723`) |
| `energy` | moment-pick energy clause + DERIVED framing | `_ENERGY_CLAUSE` → `casting_directive` (`persona_directives.py:77-78`); `_ENERGY_FRAMING` → `derive_cut_spec` framing (`:42`) → `acc.framing` → `cfg.resolve_top_bias(acct)` (`config.py:443`) → `top_bias` in render (`clip.py:310-311`), stamped on `Post.top_bias` at mint (`crosspost.py:294`) |
...
3. **A persona-link resolution failure is silent at the point of failure and only surfaces indirectly.**
   ...
   `Accounts.validate` → `differentiation_warn` when validate strings contain `"persona"` or `"shared-cut"`
   (`accounts.py:218-244`, `pipeline.py:383-385`). Evidence: the two cited spans.
