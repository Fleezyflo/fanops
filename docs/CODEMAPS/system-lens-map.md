...
Downstream, `Accounts.validate` (`accounts.py:256-264`) surfaces a "no persona linked" or "cut spec matches
...
3. **A persona-link resolution failure is silent at the point of failure and only surfaces indirectly.**
   ...
   `Accounts.validate` → `differentiation_warn` and ONLY when `account_casting` is on
   (`accounts.py:256-264`, `pipeline.py:385-387`). Evidence: the two cited spans.
