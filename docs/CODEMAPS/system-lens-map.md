...
| 3b | Ingest picks | `moments.ingest_moments` (`moments.py:161`) | agent response | `Moment` born `picked`, owner-stamped `affinities`, state→`picks_decided`; `[]` → non-terminal `moments_empty` (`moments.py:206-207`); all-invalid → `error` (`moments.py:197-198`) | none | Fail-visible: empty is LOUD but non-terminal, preserves prior moments; discards stale `moment_hooks` gates only (P11: no `moment_casting` gate, `moments.py:366`) |
...
| 4 | *(no separate stage — routing is pick-stamped)* | `casting.affinity_admits` (`casting.py:10`) gates crosspost mint + caption scope via `Moment.affinities` (stamped at pick in `ingest_moments`; operator override `cast_add`/`cast_remove`) | owner handle(s) on each moment | same `affinities` list is the sole gate input | none | `cfg.account_casting` DEFAULT ON (`config.py:593`); `=0` ignores persisted affinities and fans all |
...
| 7 | Crosspost | `crosspost.crosspost_clips` → `_mint_surface_post` (`crosspost.py:250,155`); pipeline (`pipeline.py:243`) | captioned clips, surfaces, `affinity_admits`, batch target | `Post` born `awaiting_approval` (`crosspost.py:228`), clip state→`queued` | none | Wrapped so a raise doesn't cost the pass; a FATAL `AuthError` deliberately escapes (`pipeline.py:249-255`) |
...
| 13 | `FANOPS_RESPONDER` | config.py:421 (property `:410`) | `manual` | THE explicit AI switch (llm/manual) |
...
| 27 | `FANOPS_ACCOUNT_CASTING` | config.py:599 (property `:593`) | ON | `affinity_admits` routing gate (not a separate LLM stage) |
| 28 | `FANOPS_HOOK_ROUTER` | config.py:607 (property `:603`) | OFF | Observe-only hook_strategy classifier |
| 29 | `FANOPS_IMPACT_CUT` | config.py:616 | OFF | Impact-cut stitch producer |
| 30 | `FANOPS_INTRO_TEASE` | config.py:626 | OFF | Intro-tease stitch producer |
...
global" problem string when `account_casting` is ON, and `advance` logs those as `differentiation_warn`
...
3. **A persona-link resolution failure is silent at the point of failure and only surfaces indirectly.**
   ...
   `Accounts.validate` → `differentiation_warn` when `account_casting` is ON and linked accounts share the global cut/framing spec
