...
| 3b | Ingest picks | `moments.ingest_moments` (`moments.py:161`) | agent response | `Moment` born `picked` with `affinities` stamped from `pick.personas[0]` (single owner); state→`picks_decided`; `[]` → non-terminal `moments_empty` (`moments.py:350-357`); all-invalid → `error` | none | Fail-visible: empty is LOUD but non-terminal; on reconcile discards stale `moment_hooks` gates only (`moments.py:365`) — no casting gate (removed P11) |
| 3c | Hook author (pass 2) | `moments.request_moment_hooks`/`ingest_moment_hooks` (`moments.py:394,437`) | picked window + 3 window frames + **owner-only** persona + learned styles | per-pick `moment_hooks` gate; on ingest → `Moment.hook`/`hook_removed` only (no `hooks_by_persona`), state `picked`→`decided`, source→`moments_decided` | `keyframes.extract_keyframes` over window (fail-open `[]`) | ATOMIC per source; `is_weak_hook`+`brand_risk_flag` strip mechanical/off-brand hooks, PRESERVED for restore |
| 4 | Owner routing (no LLM casting stage) | `casting.affinity_admits` (`casting.py:10`); `pipeline._owner_caption_surfaces` (`pipeline.py:146`) | `Moment.affinities` (single-owner at pick; operator `cast_add`/`cast_remove`) | crosspost + caption scope read `affinity_admits` only — no `AccountSelection`, no `_stage_casting` (removed MOL-152) | none | `cfg.account_casting` DEFAULT ON (`config.py:593-599`): OFF → admit all; `affinities==[]` → fan-to-all; non-empty → admit iff owner in set |
...
| 26 | `FANOPS_CREATIVE_VARIATION` | golive.py:225 (write-only via `_dual_write`) | **not read by `config.py`** | Studio Go-Live toggle writes `.env`+`os.environ`; runtime hardcodes `creative_variation=False` in `app.py:298` / `views.golive_status` — documentation-only until wired |
| 27 | `FANOPS_ACCOUNT_CASTING` | config.py:593-599 | ON | Single-owner `Moment.affinities` gate via `affinity_admits` (not an LLM casting stage) |
...
| `content_focus` | list[str] | `[]` | `CONTENT_FOCUS` = `_lever_vocab("content_focus")` (`personas.py:32`) | which moment KINDS to favor at pick + DERIVES cut length/framing |
| `selection_scope` | Optional[str] | None | `SELECTION_SCOPE_LEVELS` (`personas.py:33`) | pick CONSTRAINT posture (open / subject_locked / credibility_first / …) |
| `hook_angle` | Optional[str] | None | `HOOK_ANGLES` (`personas.py:34`) | on-screen hook strategy |

**Retired fields (documented in code):** `energy` stripped on load (`personas.py:76-77`, folded into `content_focus` intensity MOL-170); per-persona `clip_profile`/`framing` PINS retired M3; the 3 freeform per-dimension OVERRIDES retired M3e; `tag_lean` folded into corpus M3.
...
- **Live compose preview** — `preview_compose` (`studio/personas.py:23`) validates levers against
  `CONTENT_FOCUS/SELECTION_SCOPE_LEVELS/HOOK_ANGLES` (`studio/personas.py:32-53`); builds a TRANSIENT Persona, never
  persists.
...
|-------|-------------------------------|------------------------|
| `voice` | pick/hook/caption prompt per-owner slot | `_base_voice` (`persona_directives.py:56`) → `hook_directive` (`:82`), `caption_directive` (`:107`) → hook `personas[].persona` (`moments.py:378`) → burned `m.hook` on the clip (`crosspost.py`) |
| `content_focus` | pick SELECTION language + DERIVED cut LENGTH | `_FOCUS_CLAUSE` → `selection_directive` / pick payload (`persona_directives.py`); `_FOCUS_PROFILE` → `derive_cut_spec` → owner `clip_profile` stamped at pick (`moments.py:332`) → `render_account_cut` |
| `selection_scope` | pick CONSTRAINT posture | `_SCOPE_CLAUSE` → per-persona lenses in `request_moments` (`moments._pick_personas`) |
| `hook_angle` | on-screen hook strategy | `_ANGLE_CLAUSE` → `hook_directive` → `hook_author_slot` → hook `personas[].persona` (`moments.py:378`) → burned `m.hook` on the clip |
...
- ~~`casting_bias.casting_reach_prior`~~ — **REMOVED P11** (module deleted with LLM casting teardown).
...
(`prompts._hook_spec` `prompts.py:70`), the deterministic hashtag vetting floor, single-owner `Moment.affinities`
routing (`casting.affinity_admits`), and the smart-framing render — plus the operator approval gate. AFTER the gate opens, the reach-loop bias
...
4. **`learning_validated` is a single global boolean (`cutover.json metrics_confirmed`) that gates all
   ...
   non-degraded analyzed row from any live post unfreezes p4_dim_bias, timing_bias,
