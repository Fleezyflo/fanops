<!-- Generated: 2026-07-08 | Method: source read + deterministic AST re-sync (.reports/ast_extract.py) + hand-verified semantic corrections | Files read: 41 | Token estimate: ~7800 -->
...
`## Findings` section. Line numbers are as of the 2026-07-08 working tree (`origin/main` @ 82173c6).
...
| 3 | Request moments (pick) | `moments.request_moments` (`moments.py:261`) | transcript (char-budget bounded `moments.py:194`), peaks, `cfg.clip_profile`, 6 survey frames | opens `moments` agent gate, state→`moments_requested` | `keyframes.extract_keyframes` (fail-open `[]`) | LLM gate; per-source quarantine in pipeline (`pipeline.py:88-89`) |
| 3b | Ingest picks | `moments.ingest_moments` (`moments.py:301`) | agent response | `Moment` born `picked`, owner-stamped `affinities`, state→`picks_decided`; `[]` → non-terminal `moments_empty`; all-invalid → `error` | none | Fail-visible: empty is LOUD but non-terminal; discards stale `moment_hooks` gates only (`moments.py:365`; `moment_casting` gate removed P11) |
...
| 4 | *(no separate stage — routing is pick-stamped)* | `casting.affinity_admits` (`casting.py:10`) gates crosspost mint + caption scope via `Moment.affinities` (stamped at pick in `ingest_moments` `:340`; operator override `cast_add`/`cast_remove`) | owner handle(s) on each moment | same `affinities` list is the sole gate input | none | `cfg.account_casting` DEFAULT ON (`config.py:593`); `=0` ignores persisted affinities and fans all |
...
|-------|-------------------------------|------------------------|
| `voice` | pick/hook/caption prompt per-persona slot | `_base_voice` (`persona_directives.py:75`) → leads `casting_directive` (`:87`), `hook_directive` (`:101`), `caption_directive` (`:128`) → carried in moment-pick `personas[].persona` (`moments._pick_personas` `:220`), hook gate `personas[].persona` (`moments._hook_persona_entry` `:378`), caption `surfaces[].persona` (`caption.py:209,226`) |
| `content_focus` | moment-pick SELECTION language + DERIVED cut LENGTH | `_FOCUS_CLAUSE` → `casting_directive` (`persona_directives.py:87-99`); `_FOCUS_PROFILE` → `derive_cut_spec` (`:55`) → `resolved_cut_spec` → stamped on `Moment` at pick (`moments._stamp_owner_spec` `:286`) → `cfg.resolve_clip_profile` → `render_account_cut` band (`clip.py`) |
| `energy` | moment-pick energy clause + DERIVED framing | `_ENERGY_CLAUSE` → `casting_directive`; `_ENERGY_FRAMING` → `derive_cut_spec` framing → stamped on `Moment.framing` → `cfg.resolve_top_bias` → `top_bias` at render + `Post.top_bias` mint (`crosspost.py:294`) |
| `hook_angle` | on-screen hook strategy | `_ANGLE_CLAUSE` → `hook_directive` → `hook_author_slot` → owner-only hook gate (`moments._hook_personas_for_moment` `moments.py:384`) → `Moment.hook` → burned at render → Studio derives `variant_hook` from moment hook (no `Post.variant_hook` field post-P11) |
