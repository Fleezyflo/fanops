...
| posts per clip — uncast moment (`affinities==[]`) | **×5** — fan-to-all | `casting.py:21`; `crosspost.py:168` loop |
...
**The dedicated LLM casting stage is GONE** (P11 teardown / MOL-152). The pre-P11 casting gate
request/ingest pair, the durable selection table (`SelectionFact` side-records), and the `cast_moments`
