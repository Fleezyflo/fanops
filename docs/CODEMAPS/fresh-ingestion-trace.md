...
> Code-verified against `origin/main` @ `9c4b3b8` on 2026-07-08. Method: read `pipeline.advance()`
...
|---|---|---|
| moments per video | **M — uncapped at ingest** (no pipeline quota; `prompts._target_pick_count` is a prompt-only ceiling in `moment_pick_prompt`, not enforced at `ingest_moments`) | `moments.py:4-5,301`, `validate_pick` `:159`, `_drop_overlaps` `:145`, `prompts.py:61-68` |
