...
> Code-verified against `origin/main` @ `82173c6` on 2026-07-08. Method: read `pipeline.advance()`
...
|---|---|---|
| moments per video | **M — uncapped, model-driven** (no ingest quota; `_target_pick_count` in `prompts.py` is a prompt ceiling only, not enforced at ingest) | `moments.py:4-5,301`, `validate_pick` `:159`, `_drop_overlaps` `:145`, `prompts._target_pick_count` `:61` |
