...
> Code-verified against `origin/main` @ `0bf6ab0` on 2026-07-08. Method: read `pipeline.advance()`
...
|---|---|---|
| moments per video | **M — uncapped, model-driven** (prompt CEILING via `_target_pick_count` in `prompts.py` only; ingest has no quota — `moments.py:4-5,301`, `validate_pick` `:159`, `_drop_overlaps` `:145`) | `moments.py:4-5`, `prompts.py:61-68` |
