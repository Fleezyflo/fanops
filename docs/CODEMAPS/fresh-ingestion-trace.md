...
> Code-verified against `origin/main` @ `924e8df` on 2026-07-08. Method: read `pipeline.advance()`
...
|---|---|---|
| moments per video | **M — uncapped at ingest** (model-driven; `prompts._target_pick_count` is a **prompt ceiling only** — "up to N", never enforced in `ingest_moments`; validity + within-owner overlap dedup only) | `moments.py:4-5,301`, `prompts.py:61-68,174-180`, `validate_pick` `:159`, `_drop_overlaps` `:145` |
