...
> Code-verified against `origin/main` @ `5a4a568` on 2026-07-08. Method: read `pipeline.advance()`
...
|---|---|---|
| moments per video | **M — model-driven** (no fixed quota; `prompts._target_pick_count` is a hard CEILING in the pick prompt, not a fill target; validity + within-owner overlap dedup only) | `moments.py:4-5`, `prompts.py:61-68,174`, `validate_pick` `moments.py:159`, `_drop_overlaps` `:145` |
