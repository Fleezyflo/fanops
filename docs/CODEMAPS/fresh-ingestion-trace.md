...
| posts per clip — uncast moment (`affinities==[]`) | **×5** — fan-to-all | `casting.py:21`; `crosspost.py:228` loop (`_mint_surface_post`) |
...
request/ingest verbs, the durable per-account selection table, `SelectionFact`, and the `cast_moments`
