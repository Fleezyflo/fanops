...
| posts per clip — uncast moment (`affinities==[]`) | **×5** — fan-to-all | `casting.py:21`; `crosspost.py:228` loop |
...
**The dedicated LLM casting stage is GONE** (P11 teardown / MOL-152). The removed casting gate pair,
the durable selection records (`SelectionFact` included), and the `cast_moments`
