...
> Code-verified against `origin/main` @ `b4b632d` on 2026-07-07. Method: read `pipeline.advance()`
...
|---|---|---|
| moments per video | **M — uncapped, model-driven** (`moments.py` enforces no pick-count quota; `prompts._target_pick_count` is an optional prompt CEILING only, not a pipeline gate) | `moments.py:4-5,301`, `validate_pick` `:159`, `_drop_overlaps` `:145`, `prompts._target_pick_count` `:61` |
