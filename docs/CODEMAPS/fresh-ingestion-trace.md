...
| 3 | **Moment pick** (LLM · **opus**) | `request_moments`/`ingest_moments` (`moments.py:261/301`), frames attached | `signalled→…→moments_decided`; Moments born `picked`, **owner-stamped** | N moments (see fan-out) |
...
| 10 | Publish | `publish_due`/`publish_now` → Postiz(IG)/Zernio(TikTok) (`run.py:270`) | `queued→submitting→submitted`→**`needs_reconcile`** | poster response, submission_id |
...
|---|---|---|
| moments per video | **M — model-driven** (prompt ceiling via `prompts._target_pick_count` only; ingest validates + within-owner overlap dedup) | `prompts.py:61-68,174`, `moments.py:301`, `validate_pick` `:159`, `_drop_overlaps` `:145` |
...
| posts per clip — uncast moment (`affinities==[]`) | **×5** — fan-to-all | `casting.py:21`; `crosspost_clips` surface loop (`crosspost.py:253-271`) |
...
**The dedicated LLM casting stage is GONE** (P11 teardown / MOL-152). The casting request/ingest pair,
the durable selection side-records, and the `cast_moments` heuristic are all removed. `casting.py` header
(`casting.py:1-6`) states this explicitly.
...
| **Postiz** IG publish | `PostizPoster.publish` `postiz.py:364` `POST /public/v1/posts` ← `run._publish_one` `run.py:270` | publish | `is_live` AND provider=postiz; rate `FANOPS_POSTIZ_PUBLISH_PER_MIN` (4); `postiz_lifecycle.ensure_up` starts stack | 2xx → `submitted`+submission_id; **permalink ALWAYS None** → parks `needs_reconcile` | 401→halt; 5xx/net→`needs_reconcile` (never re-POST); other 4xx→`failed` |
