<!-- Generated: 2026-07-07 | Method: deterministic AST extraction (.reports/ast_extract.py) + derived call/import graphs (.reports/build_graphs.py) + manual semantic sync for P11/P15 casting teardown | Files scanned: 109/109 src/fanops/*.py | Regenerated: structural_index.json, import_graph.json, call_graph.json, unreferenced_candidates.json -->
...
| C4 | Moments, casting & personas | moments, casting, personas, persona_directives, persona_levers, persona_research, persona_store, accounts, batches (9) — **`casting_bias.py` removed P11** | [C4_moments_casting_personas.md](subsystem-traces/C4_moments_casting_personas.md) | 235 |
...
C7 ──validated bias artifacts (amplify-only)──> learned/transferred hooks (C5), hour_hint (C6)
...
| Bias/learning actuators are amplify-only, never touch retire/cascade/publish | C7 | **HOLDS.** Every actuator (`variant_amplify`, `p4_dim_bias`, `timing_bias`) is amplify-only + validation-frozen. (`casting_bias` removed P11.) |
...
- **Confirmed false positives** (real callers the AST tool structurally cannot see — dispatch tables,
  ...
  - `prompts.py`'s prompt-builders (`moment_pick_prompt`/`moment_hook_prompt`/`caption_prompt`) via `responder.py:_PROMPT[kind]` dict-dispatch (C5). (`moment_casting_prompt` removed P11.)
...
Totals reconcile against the deterministic count from `structural_index.json` (109 modules; callables in `call_graph.json`).
