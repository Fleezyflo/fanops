...
| `unreferenced_candidates.json` | 356 best-effort dead-code leads (excludes dunders/decorated/tests) — **leads, not verdicts**; cluster triage in each C1–C10 trace still authoritative | `.reports/` |
...
`unreferenced_candidates.json` flags 356 top-level symbols with zero in-repo callers by
name-based matching (the extractor now includes `@property` accessors and protocol methods, so the
raw count is higher than the 2026-07-07 pass — cluster triage below still holds). Each cluster triaged its own candidates by hand against actual usage
