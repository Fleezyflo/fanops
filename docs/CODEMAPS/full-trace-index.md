...
| `call_graph.json` | Name-based reverse call graph: 1,437 callables (1,144 functions + 182 methods + 111 other), each with `calls`/`called_by_in_repo` | `.reports/` |
...
`unreferenced_candidates.json` flags 354 top-level functions/methods with zero in-repo callers by
...
= 1,437 callables in `call_graph.json`, matching the AST extractor's structural index exactly
