<!-- Generated: 2026-07-08 | Method: deterministic AST extraction (scripts/codemap_extract/) + drift gate (scripts/codemap_drift.py) + hand-verified semantic sync | Files scanned: 110/110 src/fanops/*.py | Token estimate: ~2400 -->
...
| `call_graph.json` | Name-based reverse call graph: 1,342 callables, each with `calls`/`called_by_in_repo` | `.reports/` |
...
Every one of the 110 modules under `src/fanops/` is assigned to exactly one cluster below
(verified programmatically: `structural_index.json`'s 110 paths − cluster union = ∅, cluster
union − 110 paths = ∅, zero paths assigned twice).
...
| C8 | Ops, CLI & daemon | cli, daemon, doctor, cutover, cutover_postiz, health, health_model, digest, audit, timeutil, _fwrun, __init__ (12) | [C8_ops_cli_daemon.md](subsystem-traces/C8_ops_cli_daemon.md) | 320 |
...
**110/110 modules covered. 3,646 total lines of per-function trace documentation.**
...
|---|---|---|
| No-auto-publish gate (a Post can never reach a real network POST without an explicit operator approval) | C6 | **HOLDS.** Only `PostizPoster.publish`/`ZernioPoster.publish` ever touch the network, both called exclusively from `post/run.py:_publish_one`, gated on `post.state is PostState.queued`. The sole `Post(...)` construction site (`crosspost.py:228-232`) hardcodes `state=PostState.awaiting_approval`. `Ledger.approve_post` is the sole promoter to `queued` and is never called from within C6's 17 files. |
...
- **Confirmed false positives** (real callers the AST tool structurally cannot see — dispatch tables,
  ...
  - `prompts.py`'s 3 live prompt-builders (`moment_pick_prompt`/`moment_hook_prompt`/`caption_prompt`) via `responder.py:_PROMPT[kind]` dict-dispatch (C5). (The 4th casting-gate prompt builder removed P11.)
...
| C7 | 12 | ~68 |
| C8 | 12 | ~85 (37-verb CLI inventory) |
| C9 | 17 | ~150 (largest single cluster by trace length, 892 lines) |
| C10 | 5 | ~60 |

Totals reconcile against the deterministic count: 1,159 top-level functions + 183 class methods
(113 classes) = 1,342 callables in `call_graph.json`, matching the AST extractor's structural
index exactly (110/110 modules parsed with zero AST errors).
