...
| `call_graph.json` | Name-based reverse call graph: 1,326 callables (1,144 functions + 182 methods), each with `calls`/`called_by_in_repo` | `.reports/` |
...
(111 classes) = 1,326 callables in `call_graph.json`, matching the AST extractor's structural
