<!-- Generated: 2026-07-08 | Method: deterministic AST extraction (.reports/ast_extract.py) + derived call/import graphs (.reports/build_graphs.py) + hand-verified semantic sync | Files scanned: 109/109 src/fanops/*.py @ 5a4a568 | Token estimate: ~2400 -->
...
| `call_graph.json` | Name-based reverse call graph: 1,429 callables, each with `calls`/`called_by_in_repo` | `.reports/` |
...
Totals reconcile against the deterministic count: 1,144 top-level functions + 180 class methods
= 1,324 callables in the structural index (1,429 entries in `call_graph.json` incl. nested/duplicate
qualified names), matching 109/109 modules parsed with zero AST errors.
