<!-- Generated: 2026-07-08 | Method: deterministic AST extraction (.reports/ast_extract.py) + derived call/import graphs (.reports/build_graphs.py) + hand-verified semantic sync | Files scanned: 109/109 src/fanops/*.py @ c7df849 | Token estimate: ~2400 -->
...
| `call_graph.json` | Name-based reverse call graph: 1,332 callables, each with `calls`/`called_by_in_repo` | `.reports/` |
...
## Dead-code candidates (354 raw leads → triaged)

`unreferenced_candidates.json` flags 354 top-level functions with zero in-repo callers by
...
Totals reconcile against the deterministic count: 1,150 top-level functions + 182 class methods
(111 classes) = 1,332 callables in `call_graph.json`, matching the AST extractor's structural
