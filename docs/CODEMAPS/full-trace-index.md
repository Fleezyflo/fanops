<!-- Generated: 2026-07-08 | Method: deterministic AST extraction (.reports/ast_extract.py) + derived call/import graphs (.reports/build_graphs.py) + hand-verified semantic sync | Files scanned: 109/109 src/fanops/*.py @ 924e8df | Token estimate: ~2400 -->
...
| `call_graph.json` | Name-based reverse call graph: 1,320 callables, each with `calls`/`called_by_in_repo` | `.reports/` |
| `unreferenced_candidates.json` | 355 best-effort dead-code leads (excludes dunders/decorated/tests) — **leads, not verdicts**; see Dead-code below | `.reports/` |
...
Totals reconcile against the deterministic count: 1,140 top-level functions + 180 class methods
(111 classes) = 1,320 callables in `call_graph.json`, matching the AST extractor's structural
