<!-- Generated: 2026-07-09 | Method: deterministic AST extraction (scripts/codemap_extract/) + drift gate (scripts/codemap_drift.py) + hand-verified semantic sync | Files scanned: 116/116 src/fanops/*.py | Token estimate: ~2400 -->
...
| `call_graph.json` | Name-based reverse call graph: 1,444 callables, each with `calls`/`called_by_in_repo` | `.reports/` |
...
Every one of the 116 modules under `src/fanops/` is assigned to exactly one cluster below
(verified programmatically: `structural_index.json`'s 116 paths − cluster union = ∅, cluster
union − 116 paths = ∅, zero paths assigned twice).
...
**116/116 modules covered. 3,646 total lines of per-function trace documentation.**
...
- **Confirmed false positives** (real callers the AST tool structurally cannot see — dispatch tables,
  ...
  - `prompts.py`'s 3 live prompt-builders (`moment_pick_prompt`/`moment_hook_prompt`/`caption_prompt`) via `responder.py:_PROMPT[kind]` dict-dispatch (C5). (A fourth casting-gate prompt builder was removed P11.)
