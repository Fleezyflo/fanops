...
| `call_graph.json` | Name-based reverse call graph: 1,325 callables, each with `calls`/`called_by_in_repo` | `.reports/` |
...
Totals reconcile against the deterministic count: 1,143 top-level functions + 182 class methods
= 1,325 callables in `call_graph.json`, matching the AST extractor's structural index exactly
