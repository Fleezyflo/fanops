...
| `call_graph.json` | Name-based reverse call graph: 1,324 callables, each with `calls`/`called_by_in_repo` | `.reports/` |
...
Totals reconcile against the deterministic count: 1,142 top-level functions + 182 class methods
(111 classes) = 1,324 callables in `call_graph.json`, matching the AST extractor's structural
