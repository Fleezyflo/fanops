<!-- Generated: 2026-07-09 | Source: docs/CODEMAPS + docs/CODEMAPS/subsystem-traces | Maintained by hand hereafter -->
# docs/CODEMAPS — index
...
| File | Read this when… |
|---|---|
| [full-trace-index.md](full-trace-index.md) | You need the master map: 115-module coverage, the 10-cluster split, the safety-verdict table (10 invariants, all HOLD), dead-code triage, silent-failure inventory, how to regenerate. |
| [fresh-ingestion-trace.md](fresh-ingestion-trace.md) | You are running a fresh live test: one video → posts across the 5 accounts. Stage-by-stage observable states, the post-P11 single-owner fan-out arithmetic (uncapped moments × 1 aspect × affinity_admits), the external-service map (LLM/whisper/ffmpeg/Postiz/Zernio/Meta-Graph/R2 + gates), and the drop→ingest→approve→publish→verify walkthrough. Current as of 2026-07-07. |
