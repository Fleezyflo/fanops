...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  Render foundation); v6→v7 injects the empty `selection_facts` map (transient — dropped again at v11);
  v8→v9 `_migrate_v8_account_selections` lifts legacy affinities into transient `account_selections` (also
  dropped at v11); v10→v11 `_migrate_v11_drop_selection_maps` drops `account_selections` + `selection_facts`;
  ...
  variant_streaks/tag_log remain untyped (known gap). Crosspost routing reads `Moment.affinities` only
  (`casting.affinity_admits`) — no separate selection table post-v11.
