...
- Doc shape: unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  (per-account Render foundation: optional shippable artifacts) + `imported_media` (ledger-rebuild IG rows).
  Versioned: `SCHEMA_VERSION=11` + `_MIGRATIONS` hop-chain (ledger.py; v1→v2 injects the empty `stitch_plans` map;
  v2→v3 `_migrate_v3_created_at` backfills `created_at`; v3→v4 `_migrate_v4_metrics_series`; v4→v5 injects `batches`;
  v5→v6 injects `renders`; v6→v7 injects `selection_facts` (dropped again at v11); v7→v8 additive step;
  v8→v9 `_migrate_v8_account_selections` (transient lift, dropped at v11); v10→v11 `_migrate_v11_drop_selection_maps`;
  all idempotent, never raise, do NOT backfill `published_at`); a NEWER on-disk version → `_NewerSchema` refuses to load.
  New OPTIONAL entity fields (Moment.{hook_strategy, intro_matches, affinities}, StitchPlan.*, Source.{created_at, batch_id},
  Post.{created_at, published_at, batch_id}, Batch.*, Render.*) ride pydantic defaults. (`SelectionFact`/`AccountSelection`/`Post.variant_hook` removed.)
