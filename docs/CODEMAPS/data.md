...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  {created_at, published_at, batch_id}, Batch.*, Render.*) ride pydantic defaults. (`Post.variant_hook`
  and `SelectionFact` removed v11; `Moment.affinities` is the routing input.) Inner dicts of
