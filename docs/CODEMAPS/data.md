...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  Render foundation); v6→v7 injects the empty `selection_facts` map (M4 — **dropped again at v10→v11**, P12/MOL-154);
  v7→v8 … v9→v10 additive hops; **v10→v11** `_migrate_v11_drop_selection_maps` removes the retired
  `account_selections` + `selection_facts` maps — crosspost gate is `Moment.affinities` only). All idempotent,
  never raise, do NOT backfill `published_at` — old ledgers load clean. A NEWER on-disk version →
  ...
  {created_at, published_at, batch_id}, Batch.*, Render.*) ride pydantic defaults. (`Post.variant_hook` and
  `SelectionFact`/`AccountSelection` were **removed** — hook lives on `Moment.hook`; Studio `variant_hook` is a
  read-only UI projection of `Render.hook_text` / `Moment.hook`.) Inner dicts of
