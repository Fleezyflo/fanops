...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  Render foundation); v6→v7 injects the empty `selection_facts` map (M4 filing/naming/tracking); v7→v8 identity
  stamp for Render provenance scalars; v8→v9 `_migrate_v8_account_selections` lifts legacy affinities into
  durable selection maps (later dropped); v9→v10 injects `imported_media`; v10→v11 `_migrate_v10_drop_selections`
  drops the retired selection maps (P12/MOL-154); all idempotent, never raise, do NOT backfill
