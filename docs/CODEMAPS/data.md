...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  Render foundation); v6→v7 / v8→v9 transiently inject `selection_facts` / `account_selections` for upgrading
  old ledgers; v10→v11 `_migrate_v11_drop_selection_maps` drops those retired maps (P12/MOL-154); all idempotent, never raise, do NOT backfill
  ...
  durable selection-fact models **removed** — hook on `Moment.hook` / `Render.hook_text`.) Inner dicts of
