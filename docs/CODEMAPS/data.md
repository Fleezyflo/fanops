...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  latest additive step; v8→v9 lifts legacy affinities into transient `account_selections`; v9→v10 and v10→v11
  drop `account_selections` + `selection_facts` (P12/MOL-154); all idempotent, never raise, do NOT backfill
