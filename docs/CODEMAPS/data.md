...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  (per-account Render foundation: the per-account shippable artifacts) + `imported_media` (live IG posts
  probed with no clip lineage). Versioned:
  `SCHEMA_VERSION=11` + `_MIGRATIONS` hop-chain (ledger.py; v1→v2 injects the empty `stitch_plans` map;
  v2→v3 `_migrate_v3_created_at` backfills `created_at`; v3→v4 `_migrate_v4_metrics_series` back-fills ONE
  'legacy'-tagged metrics_series row; v4→v5 injects `batches`; v5→v6 injects `renders`; v6→v7 injects
  `selection_facts` (later dropped); v7→v8 identity stamp; v8→v9 injects `account_selections` (later dropped);
  v9→v10 injects `imported_media`; v10→v11 `_migrate_v10_drop_selections` drops `account_selections` +
  `selection_facts` (P12/MOL-154 — crosspost reads `Moment.affinities` only). All hops idempotent, never raise,
  do NOT backfill `published_at` — old ledgers load clean); a NEWER on-disk version →
  ...
  {created_at, published_at, batch_id, variant_hook}, Batch.*, Render.*, ImportedMedia.*) ride pydantic defaults. Inner dicts of
