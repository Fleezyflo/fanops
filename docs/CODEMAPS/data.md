...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  `published_at` — old ledgers load clean, proven on the real 51-post ledger); v10→v11 drops retired
  `account_selections` + `selection_facts` maps (P12/MOL-154). A NEWER on-disk version →
