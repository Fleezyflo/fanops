...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  (per-account Render foundation) + `imported_media` (ledger-rebuild IG projection). Versioned:
  `SCHEMA_VERSION=11` + `_MIGRATIONS` hop-chain (ledger.py; v1→v2 injects `stitch_plans`; v2→v3
  `_migrate_v3_created_at` backfills `created_at`; v3→v4 `_migrate_v4_metrics_series` back-fills ONE
  'legacy'-tagged metrics_series row; v4→v5 injects `batches`; v5→v6 injects `renders`; v6→v7 injects
  `selection_facts` (transient — existed v7–v10 only); v7→v8 identity stamp (Render provenance scalars ride
  pydantic defaults); v8→v9 `_migrate_v8_account_selections` lifts legacy affinities into transient
  `account_selections`; v9→v10 identity; **v10→v11 `_migrate_v10_drop_selections` DROPS** the retired
  `account_selections` + `selection_facts` maps (P12/MOL-154 — crosspost gate is `Moment.affinities` only).
  All hops idempotent, never raise, do NOT backfill `published_at`; a NEWER on-disk version → `_NewerSchema`
  refuses to load (exit 2). New OPTIONAL entity fields (Moment.{hook_strategy, intro_matches, affinities},
  StitchPlan.*, Source.{created_at, batch_id}, Post.{created_at, published_at, batch_id, variant_hook},
  Batch.*, Render.*, ImportedMedia.*) ride pydantic defaults. (`SelectionFact`/`AccountSelection` models
  removed v11.) Inner dicts of variant_streaks/tag_log remain untyped (known gap).
