...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  Render foundation); v6→v7 injects the empty `selection_facts` map (M4 filing/naming/tracking); v7→v8 additive
  steps; v9→v10 lift durable selection maps; v11 `_migrate_v10_drop_selections` drops retired selection maps
  (P12/MOL-154); all idempotent, never raise, do NOT backfill
