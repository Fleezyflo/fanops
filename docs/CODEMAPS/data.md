...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  (per-account Render foundation: optional per-account shippable artifacts). Versioned:
  ...
  Render foundation); v6→v7 injects the empty `selection_facts` map (M4, historical); v7→v8 additive;
  v8→v9 injects `account_selections` (RF1, historical); v9→v10 AccountSelection sum-type migration;
  v10→v11 **drops** the retired `account_selections` + `selection_facts` maps (P12/MOL-154 — routing is
  `Moment.affinities` + `affinity_admits` only); all idempotent, never raise, do NOT backfill
  `published_at` — old ledgers load clean); a NEWER on-disk version →
  `_NewerSchema` refuses to load (exit 2) rather than silently drop fields. New OPTIONAL entity fields
  (Moment.{hook_strategy, intro_matches, affinities, clip_profile, framing}, StitchPlan.*,
  Source.{created_at, batch_id}, Post.{created_at, published_at, batch_id, render_id, top_bias, …},
  Batch.*, Render.*) ride pydantic defaults. (`Post.variant_hook`, `AccountSelection`, `SelectionFact`
  removed — hook lives on `Moment.hook`; routing on `Moment.affinities`.) Inner dicts of
