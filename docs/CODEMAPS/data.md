...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  (per-account Render foundation: the per-account shippable artifacts) + `imported_media` (M1 asset memory).
  Versioned:
  ...
  `selection_facts` (transient — dropped at v11); v7→v8 identity stamp; v8→v9 lifts legacy affinities into
  transient `account_selections` (also dropped at v11); v10→v11 `_migrate_v10_drop_selections` drops
  `account_selections` + `selection_facts` (P12/MOL-154). All hops idempotent; a NEWER on-disk version →
  `_NewerSchema` refuses to load (exit 2). New OPTIONAL entity fields
...
- **personas.json:** first-class `Persona` records (`models`/`personas.py`) — `voice`/`hashtag_corpus` + lever fields (`tag_lean` retired M3, folded into corpus)
