...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  (per-account Render foundation: the per-account shippable artifacts) + `imported_media`. Versioned:
  ...
  `batches`; v5→v6 injects `renders`; v6→v7 injects `selection_facts` (historical — dropped again at v11);
  v7→v8 additive step; v8→v9 `_migrate_v8_account_selections` (transient lift); v10→v11
  `_migrate_v11_drop_selection_maps` drops `account_selections` + `selection_facts`; all idempotent, never raise).
  A NEWER on-disk version → `_NewerSchema` refuses to load (exit 2). New OPTIONAL entity fields
  (Moment.{hook_strategy, intro_matches, affinities}, StitchPlan.*, Source.{created_at, batch_id}, Post.
  {created_at, published_at, batch_id, render_id, first_frame_kind, clip_profile, …}, Batch.*, Render.*) ride
  pydantic defaults. (`Post.variant_hook` / `SelectionFact` removed P9/P12.) Inner dicts of variant_streaks/tag_log
...
- **personas.json:** first-class `Persona` records (`models`/`personas.py`) — `voice`/`hashtag_corpus`
  per persona; `Account.persona_id` links one and its voice/corpus HYDRATE the account at load (fail-open,
