...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  Render foundation); v6→v7 injected the empty `selection_facts` map (M4, historical — dropped again at v11);
  v7→v8 the additive step; v8→v9 `_migrate_v8_account_selections` lifted legacy affinities into transient
  `account_selections` (historical — dropped at v11); v10→v11 `_migrate_v11_drop_selection_maps` removes
  `account_selections` + `selection_facts`; all idempotent, never raise, do NOT backfill
...
- **personas.json:** first-class `Persona` records (`models`/`personas.py`) — `voice`/`hashtag_corpus` + lever
  fields per persona; `tag_lean` was folded into the corpus (M3). `Account.persona_id` links one and its
  voice/corpus HYDRATE the account at load (fail-open, byte-identical when unlinked). Edited in the Studio
  Personas tab; mutated under `personas.lock` (reuses the ledger flock shape).
