...
- Doc shape: 4 unit maps keyed by content-addressed id + `variant_streaks` + `tag_log` + `stitch_plans`
  ...
  Render foundation); v6→v7 injects the empty `selection_facts` map (M4, **dropped v10→v11 / P12**);
  v7→v9 injects `account_selections` (RF1, **dropped v10→v11 / P12**); v10→v11
  `_migrate_v11_drop_selection_maps` removes both retired maps. All hops idempotent, never raise, do NOT
  backfill `published_at` — old ledgers load clean); a NEWER on-disk version → `_NewerSchema` refuses to
  load (exit 2) rather than silently drop fields. New OPTIONAL entity fields
  (Moment.{hook_strategy, intro_matches, affinities}, StitchPlan.*, Source.{created_at, batch_id}, Post.
  {created_at, published_at, batch_id, render_id, top_bias, …}, Batch.*, Render.*) ride pydantic defaults.
  (`Post.variant_hook` and `SelectionFact`/`AccountSelection` were removed — owner hook on `Moment.hook`,
  burned hook on `Render.hook_text`.) Inner dicts of
