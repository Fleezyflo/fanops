...
| Autonomous ops | autopilot.py (one-cmd: enable llm responder + launchd daemon), daemon.py (launchd supervisor around `run`), doctor.py (readiness pre-flight checks), cutover.py (Blotato: auth/post/metrics/lift prover) + cutover_postiz.py (Postiz throwaway-probe prover) — both OPTIONAL early shortcuts: learning auto-unfreezes on the first real non-degraded live metric (track._auto_validate_metrics_shape), so the cutover probe is no longer required |
