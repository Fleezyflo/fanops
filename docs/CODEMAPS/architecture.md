...
Account-First Studio (SCHEMA_VERSION 11; all default-safe): a **Batch** is a named, account-targeted ingest grouping (`Batch.target_accounts`; `[]` = all-active sentinel). `create_batch` mints a content-addressed id and (when an active-handle set is supplied) flags a zero-active-target batch via `Batch.error_reason` (advisory — still mints). The target is denormalized onto `Source.batch_id` → `Post.batch_id` and ENFORCED by a per-surface SKIP in `crosspost_clips` so `target_accounts=['@a']` is not a silent no-op. The default-ON `FANOPS_ACCOUNT_CASTING` flag gates `casting.affinity_admits` — owner-stamped `Moment.affinities` at pick time (no LLM casting stage; removed P11/MOL-152). New surfaces: **Home**, batch-grouped **Review** (cast chips from `Moment.affinities`, operator `cast_add`/`cast_remove`), paginated **Schedule/Posted** with `?batch=` filter, Go-Live **casting toggle**. Learning stays validation-frozen behind `learning_validated`. The orphan flags `account_first`/`batch_studio` were never built (dropped, not deferred).
