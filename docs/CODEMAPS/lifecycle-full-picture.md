...
> **Historical context (post-P11 / MOL-152):** Findings 1, 3, and the Stage 5 casting narrative below reference the **removed** LLM casting stage and `account_selection_admits`. The live model is `casting.affinity_admits` + pick-stamped `Moment.affinities` — see [fresh-ingestion-trace.md](fresh-ingestion-trace.md) §3.
