...
- ~~`views_posted.py` `lineage_stats` in-place mutation~~ — **CLOSED (MOL-70 parity).** `lineage_stats` and `account_median_deltas` now return new annotated lists via `dataclasses.replace`; fail-open paths log via `logger.warning` and return the input rows unchanged.
