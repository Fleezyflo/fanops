...
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure **prompt ceiling helper** (not a pipeline quota — `moments.py` has no `_target_pick_count`; ingest accepts however many valid picks the model returns). Computes an "up to N" ceiling for `moment_pick_prompt` by source length and band.
