...
- `_MAX_TARGET_PICKS = 30` (module var) — the hard CEILING in the moment-pick **prompt** (never a quota in `ingest_moments`; moments are uncapped/model-driven at ingest — see [fresh-ingestion-trace.md](../fresh-ingestion-trace.md) §2).
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure: computes a **prompt ceiling** for how many non-overlapping clips to aim for, by source length and content-type band. `<=0` duration -> `0` (no target, model decides); below the band floor -> `1` (one whole-source clip); else `round(duration / band.span)`, floored at 1, capped at `_MAX_TARGET_PICKS`. Called by `moment_pick_prompt` only — NOT an ingest cap.
...
- `LlmSchemaError(RuntimeError)` — class; **MOL-228** — typed exception for unparseable/non-conformant `claude -p` JSON envelope output (subclass of `RuntimeError` so existing `raises(RuntimeError)` tests stay green). Defined at `llm.py:42`; responder wiring to catch it and stamp `degraded_reason` is a follow-up — today schema failures still fall through the generic `except Exception` in `responder._answer_one`.
