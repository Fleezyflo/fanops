...
The SAME dispatch shape is used by `moments.py` (outside this cluster, in C4) for the `moments`/`moment_hooks` gates, whose prompt builders (`moment_pick_prompt`, `moment_hook_prompt`) live in `prompts.py` — this is why `call_graph.json` shows `called_by_in_repo: []` for those functions and for `caption_prompt`: the AST-based call graph cannot trace `_PROMPT[kind](payload)` (a dict-value indirect call in `responder.py`) back to the literal function name. Confirmed real callers via grep: `responder.py:47-48,66` binds `_PROMPT = {"moments": moment_pick_prompt, "moment_hooks": moment_hook_prompt, "captions": caption_prompt}` and calls `_PROMPT[kind](payload)`. (`moment_casting_prompt` removed P11/MOL-152.) This is **not dead code** — see Anomalies for how this is distinguished from genuine dead code in this cluster.
...
### `prompts.py` — the committed prompt-builder catalog (one function per live agent-gate kind)

- `_brief_fence(guidance) -> str` — pure: wraps operator brand guidance in a `<brand_brief>...</brand_brief>` fence. Called by `moment_pick_prompt`, `moment_hook_prompt`, `caption_prompt`.
- `_inline(s) -> str` — pure whitespace collapse for injection hardening. Called by `moment_hook_prompt`.
- `_MAX_TARGET_PICKS = 30` — prompt CEILING only (not a pipeline quota); `moments.py` has no pick-count enforcement.
- `_target_pick_count(duration, band)` — pure helper for the pick prompt's optional "up to N" ceiling on long sources; `<=0` duration → no ceiling line (model-driven). Called by `moment_pick_prompt` only.
- `_hook_spec` / `_hook_decision` — shared hook craft + selection logic. Called by `moment_hook_prompt`.
- `moment_pick_prompt(payload)` — **PASS 1** window-picking prompt. Called via `responder._PROMPT["moments"]`.
- `moment_hook_prompt(payload)` — **PASS 2** owner-only hook author (one `m.hook` per moment). Called via `responder._PROMPT["moment_hooks"]`.
- `caption_prompt(payload)` — caption/hashtag prompt. Called via `responder._PROMPT["captions"]`.
- **Removed P11:** `moment_casting_prompt`, `_casting_moment_line`, casting-specific `_data_fence` consumers.
...
- `src/fanops/prompts.py` `moment_pick_prompt`, `moment_hook_prompt`, `caption_prompt` — zero-caller in `call_graph.json` because of `responder._PROMPT[kind]` dict-dispatch (`responder.py:47-48,66`). All three are live. (`moment_casting_prompt` removed P11.)
