...
Cross-checked against `.reports/structural_index.json` and `.reports/call_graph.json`. `responder.py` wires `prompts.py` builders via `_PROMPT[kind]` dict-dispatch (zero direct call-graph edges — not dead code).
...
The SAME dispatch shape is used by `moments.py` (C4) for the `moments`/`moment_hooks` gates, whose prompt builders (`moment_pick_prompt`, `moment_hook_prompt`) live in `prompts.py` — `moment_casting_prompt` was **removed P11**. Dict-dispatch via `responder._PROMPT[kind](payload)` is why the call graph shows zero direct callers for those builders.
...
- `_brief_fence(guidance) -> str` — Called by `caption_prompt`, `moment_hook_prompt`, `moment_pick_prompt`.
- `_inline(s) -> str` — Called by `moment_hook_prompt` (and historical casting helpers, removed P11).
- `_data_fence(label: str, body: str) -> str` — **REMOVED callers** with `moment_casting_prompt` (P11).
...
- `moment_hook_prompt(payload: dict) -> str` — **M1b PASS 2 prompt builder**: owner-only `personas[]` entry (P6), `_hook_decision` + `_hook_spec`, optional `learned_hooks`. No `hooks_by_persona` block (removed with per-account hook maps). Called via `responder._PROMPT["moment_hooks"]`.
- ~~`moment_casting_prompt`~~ — **REMOVED P11** (LLM casting stage deleted).
- ~~`_casting_moment_line` / `moment_casting_prompt`~~ — **REMOVED P11**.
...
- `src/fanops/prompts.py` `moment_pick_prompt`, `moment_hook_prompt`, `caption_prompt` — `called_by_in_repo: []` via `responder._PROMPT` dict-dispatch; load-bearing, not dead.
