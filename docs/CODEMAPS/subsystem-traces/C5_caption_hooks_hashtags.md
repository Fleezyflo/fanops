...
> **Not the ship path.** The function catalog below is a snapshot of deleted Layer B
> (`vet_hashtags` / `_caption_entry`). Live ship is `docs/CODEMAPS/hashtag-lifecycle.md`
> (`ship_from_lock`). Live vendor `content` is `src/fanops/post/CLAUDE.md`
> (`posted_text_for` / `compose_posted_caption`).
...
- `ranked_tags(measurements) -> list[str]` — cache as metric-DESC ordered menu (`hashtags.py:~101`).
- `content_tag_candidates(text, *, max_n=6) -> list[str]` — pure, deterministic, NO NLP (`hashtags.py:~117`): Latin word tokens → `#tag` candidates that still must pass membership. Called by `caption.request_captions`.
- `_screen_content(content_norm, cfg=None)` — drops brand-risk content candidates before membership.
- `vet_hashtags(tags, platform, language=None, max_tags=4, *, store=None, corpus=None, content=None, cfg=None, recent=None) -> list[str]` — **the selection algorithm** (`hashtags.py:~153-230`). Membership = store ∪ corpus ∪ content; composition only (cap 4, corpus lead, graded LRU, `_ARABIC`); backfill corpus → store → content; cold empty. Called by `vet_hashtags_traced`, `persona_directives.persona_facts`.
...
- `refresh_store_if_due(cfg, *, max_age_s=43200, ...)` — 12h mtime throttle; no Meta creds → clean no-op; fail-open on errors; reports corrupt-personas abort. Called by `fanops run` tick.
- `cmd_hashtags_refresh(cfg) -> int` — run a measurement pass now; exit 2 only on corrupt-personas abort. Called by `cli._dispatch`.
- `cmd_hashtags_discover(cfg) -> int` — READ-ONLY / ZERO NETWORK niche report via `derived_report` (projects the cache); always exits 0. Called by `cli._dispatch`.

Confirms: `fanops_hashtags.py` is NOT part of the caption gate — the caption path consumes the measured cache via `hashtags.load_measurements` / `ranked_tags`.
...
- `_hook_spec(max_words: int = 6) -> str` — pure: the SHARED on-screen-hook craft specification (retention psychology: ~70% watch muted, decide in <3s; the 4 proven triggers — curiosity gap, pattern interrupt, self-relevance, emotional arousal — plus 5 situational mechanisms, force multipliers, worked examples, banned anti-patterns, bilingual guidance, output constraints). Reused verbatim by both `moment_hook_prompt` (the vision hook author) and (per its own docstring reference) `caption_prompt`'s dormant variant machinery is NOT wired to it — confirmed `caption_prompt` does not call `_hook_spec` (grep of `prompts.py` shows only `moment_hook_prompt` calls it); ingest HOLD and live send are different; send is `posted_text_for`. Called by `moment_hook_prompt`.
