...
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure: computes a **prompt-only CEILING** ("up to N" in `moment_pick_prompt`, never a hard ingest quota). `<=0` duration -> `0`; below band floor -> `1`; else `round(duration / band.span)`, floored at 1, capped at `_MAX_TARGET_PICKS`. `ingest_moments` does NOT enforce this count — validity + within-owner overlap dedup only (`moments.py:4-5`). Called by `moment_pick_prompt`.
