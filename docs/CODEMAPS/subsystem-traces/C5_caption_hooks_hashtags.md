...
- `_MAX_TARGET_PICKS = 30` (module var) — the hard CEILING in the pick **prompt** (never a quota, never enforced by `moments.ingest_moments`).
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure **prompt-ceiling helper** (NOT a pipeline pick-count cap). `<=0` duration → `0` (no "up to N" line); below the band floor → `1`; else `round(duration / band.span)`, floored at 1, capped at `_MAX_TARGET_PICKS`. Called only by `moment_pick_prompt` to phrase the brief; actual fan-out is model-driven + `validate_pick`/`_drop_overlaps` in `moments.py`.
