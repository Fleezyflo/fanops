...
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure: computes a prompt **ceiling** ("up to N", never a quota) by source length and content-type band. `<=0` duration -> `0` (no ceiling line, model decides); below the band floor -> `1` (one whole-source clip); else `round(duration / band.span)`, floored at 1, capped at `_MAX_TARGET_PICKS`. Called by `moment_pick_prompt`.
