...
The SAME dispatch shape is used by `moments.py` (outside this cluster, in C4) for the `moments`/`moment_hooks`/`moment_casting` gates, whose prompt builders (`moment_pick_prompt`, `moment_hook_prompt`, `moment_casting_prompt`) also live in `prompts.py` — this is why `call_graph.json` shows `called_by_in_repo: []` for those three functions and for `caption_prompt`: the AST-based call graph cannot trace `_PROMPT[kind](payload)` (a dict-value indirect call in `responder.py`) back to the literal function name. Confirmed real callers via grep: `responder.py:19,28-29,50` binds `_PROMPT = {"moments": moment_pick_prompt, "moment_hooks": moment_hook_prompt, "moment_casting": moment_casting_prompt, "captions": caption_prompt}` and calls `_PROMPT[kind](payload)` at `responder.py:50`. This is **not dead code** — see Anomalies for how this is distinguished from genuine dead code in this cluster.
...
- `_brief_fence(guidance) -> str` — pure: wraps operator brand guidance (from `context.md`) in a delimited `<brand_brief>...</brand_brief>` fence, framed explicitly as REFERENCE DATA never instructions; any embedded fence-tag is collapsed to an inert token first. Empty/None guidance -> `"(none provided)"`. Documented residual: the brand brief is operator-authored third-person artist bio and is the one priming source viewer-POV starvation cannot fully neutralize (mitigated, not guaranteed, by the hook rule + the read-only `narration_signature` meter). Called by all 4 prompt builders (`caption_prompt`, `moment_casting_prompt`, `moment_hook_prompt`, `moment_pick_prompt`).
- `_inline(s) -> str` — pure: collapses a string's whitespace (CR/LF/TAB included) to single spaces — AGENT-3 hardening so an untrusted free-text channel (a prior gate's model-written reason/hook, an account persona voice string) can never start a new flush-left/bulleted line inside a later prompt (the same structural injection guard `json.dumps` gives the transcript). Called by `_casting_moment_line`, `moment_casting_prompt`, `moment_hook_prompt`.
- `_DATA_FENCE_TAG` (module var, compiled regex) — matches any `<source_data>`/`</source_data>` tag.
- `_data_fence(label: str, body: str) -> str` — pure: the casting-prompt analog of `_brief_fence` — wraps untrusted blocks (account personas, model-written moment reasons/hooks/transcript) in a `<source_data>` fence labeled DATA-only, collapsing any forged fence tag first; empty body -> `"(none)"`. Called by `moment_casting_prompt`.
...
- `moment_hook_prompt(payload: dict) -> str` — **M1b PASS 2 prompt builder**: assembles the frame-grounded on-screen-hook-authoring prompt for ONE already-picked window — clip start/end/duration, the pick `reason` (framed as source-to-transform, never to echo), hard rules, `_hook_decision` + `_hook_spec` (the full craft spec), an optional `learned_hooks` STYLE-cue block (P4c bias, present only when non-empty), an optional per-account `hooks_by_persona` block (keyed by exact handle string, each account's own voice, each independently obeying every hook rule), signal peaks, `_brief_fence`, language, transcript excerpt. Called via `responder._PROMPT["moment_hooks"]`.
- `_casting_moment_line(m: dict) -> str` — pure: formats one candidate moment's summary line for the casting prompt (id, window, signal score, reason, optional hook/transcript/frame-attached marker). Called by `moment_casting_prompt`.
- `moment_casting_prompt(payload: dict) -> str` — **M1 Option C prompt builder**: assembles the per-account moment-SELECTION prompt — moment lines (`_casting_moment_line`), persona lines per account, an optional `learned` (prior-taste) block, an optional frame-attached note, hard rules (fit-by-persona, be generous/no cap, use exact handle/moment_id strings, leave an account empty only when nothing fits), `_brief_fence`, language, and `_data_fence`-wrapped accounts/prior-taste/moments blocks. Called via `responder._PROMPT["moment_casting"]`.
- `caption_prompt(payload: dict) -> str` — **the text-only caption/hashtag prompt builder**: assembles the fan-voice-not-artist framing, per-surface language declaration+validation rule, exactly-one-item-per-surface rule (never refuse even on explicit lyrics — captions are hashtags-only so content sensitivity is irrelevant), the hashtags-only `caption` format rule (<=4 tags), the reach-vetted menu (`vetted_menu()`) with a widened pick_rule when `content_tags` is present (menu UNION clip-specific tags), the balanced-4 composition guidance, per-surface `persona`/`corpus` honoring instructions, optional `learned_hooks`/`learned_hooks_transferred` STYLE-cue blocks, `_brief_fence`, transcript excerpt, and the JSON-serialized `surfaces` list. Called via `responder._PROMPT["captions"]`, and independently by `studio.actions.regenerate_caption` (a manual Studio action reusing the production prompt).
...
- `src/fanops/prompts.py:187` `moment_pick_prompt`, `:242` `moment_hook_prompt`, `:318` `moment_casting_prompt`, `:361` `caption_prompt` — all show `called_by_in_repo: []` in `call_graph.json` because they are invoked via `responder.py`'s `_PROMPT[kind](payload)` dict-dispatch (`responder.py:28-29,50`), which the AST-based call graph cannot trace back to the literal function names. All four are genuinely load-bearing — this is the exact "check call_graph.json, but verify" trap the C2/C4 reports also flagged for other dispatch-table patterns in this codebase.
