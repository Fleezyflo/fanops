...
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure: computes a **CEILING** (not a quota) for how many non-overlapping clips the pick prompt may aim for, by source length and content-type band. `<=0` duration -> `0` (no target ceiling in prompt, model decides freely); below the band floor -> `1` (one whole-source clip); else `round(duration / band.span)`, floored at 1, capped at `_MAX_TARGET_PICKS` (30). Called by `moment_pick_prompt`. Moments remain uncapped/model-driven at ingest — validity + within-owner overlap dedup only (`moments.py`; no hard pick-count gate).
...
- `LlmSchemaError(RuntimeError)` — class; raised when the envelope has no parseable `structured_output` or JSON `result` after salvage + (for vision gates) the MOL-248 schema-only finalizer retry.
...
- `claude_json_meta(prompt: str, schema: dict, *, timeout: float = 300.0, images: list[str] | None = None, model: str | None = None) -> tuple[dict, str | None, bool]` — **THE ACTUAL LLM-CALLING PRIMITIVE.** Builds the `claude -p --output-format json --json-schema <schema> --allowedTools <"Read" if images else ""> --strict-mcp-config [--model <model>]` argv; runs it via an inner `_run(stdin_prompt, allowed=...)` closure that shells `subprocess.run` with the prompt piped on **stdin** (not argv — ECC fix #11, avoiding `ps`/`/proc/<pid>/cmdline` leakage of the transcript/brand-guidance and avoiding `ARG_MAX`/E2BIG on a large transcript), wrapped in a jittered-exponential rate-limit retry loop (`_rate_limit_status`-gated, up to `_MAX_RL_RETRIES`=4 retries, raising `LlmRateLimitError` on exhaustion). Absent binary -> `ToolchainMissingError`; hang past `timeout` -> `LlmTimeoutError`; nonzero non-rate-limited exit -> `LlmContextLimitError` (if `_is_context_limit` matches the error body) or generic `RuntimeError`; unparseable stdout -> `LlmSchemaError`. When `images` is given: sends a first attempt instructing the model to read the frames via the Read tool, checks `_frames_unread`; if unread, re-asks ONCE more forcibly; if STILL unread after the re-ask, proceeds anyway but sets `frames_unread=True` (surfaced to the responder as a degraded, text-grounded-not-frame-grounded breadcrumb, AGENT-9) and logs a warning. Resolves via `_resolve_from_env`: prefers `structured_output`, else `json.loads(result)`, else `_extract_json_object` salvage (MOL-241). **MOL-248 vision finalizer:** when frames were attached and `result` is prose with no salvageable JSON (`repair_empty`), re-runs once with `--allowedTools ""` and a schema-only instruction (`Respond with ONLY a single JSON object…`) before raising `LlmSchemaError`. Returns `(parsed_dict, model_that_answered_or_pinned_fallback, frames_unread)`. Called by `responder._default_claude_model`.
