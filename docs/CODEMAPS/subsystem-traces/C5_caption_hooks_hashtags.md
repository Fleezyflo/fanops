...
9. `src/fanops/llm.py` (243 lines) — read
...
> **Removed P11:** `moment_casting_prompt`, `_casting_moment_line` (casting LLM gate deleted). `_data_fence` remains — used by `moment_pick_prompt` / `moment_hook_prompt` for untrusted persona/moment text blocks (`prompts.py:50-53,116-120,205`).
...
- `_JSON_FENCE_RE` (module var, compiled regex) — matches ` ```json … ``` ` fenced blocks for JSON extraction (`llm.py:29`).
- `_json_candidates(text: str) -> list[str]` — pure: returns candidate JSON-object strings from `text` in priority order — fenced `json` blocks first, then balanced-brace outermost `{…}` scans respecting string escaping (`llm.py:31-65`, MOL-237). Called by `_extract_json_object`.
- `_extract_json_object(text: str) -> dict | None` — pure: iterates `_json_candidates`, returns the first parseable JSON dict or `None` (`llm.py:67-78`, MOL-237). Infrastructure for resilient envelope parsing; the live `claude_json_meta` path still prefers `structured_output` then `json.loads(result)`.
...
- `LlmSchemaError(RuntimeError)` — class; raised when `claude -p` output cannot be parsed/validated into the requested JSON schema (unparseable envelope, non-object envelope, non-JSON `result`, or no structured payload). Typed so the responder stamps a visible degraded gate state on schema failure instead of a generic `RuntimeError` (`llm.py:93-98`).
...
- `src/fanops/llm.py:120` `except Exception:` in `_rate_limit_status` — any envelope-parse failure -> `None` (not a rate-limit signal), falls through to the hard-failure path, which is itself surfaced (not swallowed).
- `src/fanops/llm.py:199` `except Exception as e:` in `claude_json_meta`'s `_run` — stdout-JSON-parse failure re-raised as `LlmSchemaError` with the raw output tail (not swallowed).
- `src/fanops/llm.py:234` `except Exception as e:` in `claude_json_meta` — the `result`-string JSON-parse fallback re-raised as `LlmSchemaError`, not swallowed.
