...
- `_MAX_TARGET_PICKS = 30` (module var) — the hard CEILING in the moment-pick **prompt** (never an ingest quota; `moments.py` validates whatever the model returns).
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure: computes the prompt's "up to N" ceiling by source length and content-type band (`prompts.py:61-68`). `<=0` duration -> `0` (no ceiling line in the prompt); below the band floor -> `1`; else `round(duration / band.span)`, floored at 1, capped at `_MAX_TARGET_PICKS`. Called only by `moment_pick_prompt` — **not** enforced at `ingest_moments` (moments are uncapped/model-driven per `moments.py:4-5`).
