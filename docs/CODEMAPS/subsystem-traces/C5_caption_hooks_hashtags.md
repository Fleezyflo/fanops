...
- `LlmSchemaError(RuntimeError)` — class; MOL-233 — raised when `claude -p` stdout cannot be parsed into the requested JSON envelope/`structured_output` (unparseable envelope, non-object envelope, non-JSON `result`, or missing payload). Subclass of `RuntimeError` so existing `raises(RuntimeError)` tests stay green; the responder currently catches it via the generic `except Exception` path (logs `error`, gate stays pending).
