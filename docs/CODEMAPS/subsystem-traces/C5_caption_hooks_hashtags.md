...
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure: computes how many non-overlapping clips to aim for in the **pick prompt** (a ceiling hint to the model, NOT an ingest hard cap — ingest accepts any valid pick count). `<=0` duration -> `0` (no target, model decides); below the band floor -> `1` (one whole-source clip); else `round(duration / band.span)`, floored at 1, capped at `_MAX_TARGET_PICKS`. Called by `moment_pick_prompt`.
