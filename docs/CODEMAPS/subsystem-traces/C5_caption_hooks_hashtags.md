...
- `_tag_source(tag, *, content_set, corpus_set, store_set) -> str` — pure: the provenance label for one shipped tag, priority order content > corpus > region > graph-reach > discovery. Called by `vet_hashtags_traced`.
