...
9. `src/fanops/llm.py` (244 lines) — read
...
- `_JSON_FENCE_RE` (module var, compiled regex) — matches ```json fenced blocks for `_json_candidates`.
- `_json_candidates(text) -> list[str]` — pure: priority-ordered JSON-object substring candidates (fenced blocks, then balanced-brace scan respecting string escaping). Called by `_extract_json_object`.
- `_extract_json_object(text) -> dict | None` — pure: first valid JSON dict from `_json_candidates`, else `None`. Called by (internal recovery paths in tests; primary path is envelope parsing in `claude_json_meta`).
...
- `LlmSchemaError(RuntimeError)` — class; MOL-227 — raised when `claude -p` output cannot be parsed/validated into the requested JSON schema (unparseable envelope, non-object envelope, non-JSON `result`). Typed so the responder routes schema failures through `_on_deterministic_fail` instead of an infinite-pending generic `RuntimeError`.
- `_CONTEXT_LIMIT_MARKERS` (module var, tuple of strings) — substrings that identify a context-limit failure in the CLI's stderr/stdout body.
...
- `_rate_limit_status(returncode: int, stdout: str) -> int | None` — pure: returns the retryable rate-limit status if this result is one, else `None`. Called by `claude_json_meta`.
- `_frames_unread(env: dict) -> bool` — pure: HOOK-TRANSPORT check — True iff the response envelope's `num_turns` is an int `<= 1`. Called by `claude_json_meta`.
- `claude_json_meta(...)` — **THE ACTUAL LLM-CALLING PRIMITIVE.** Builds argv via inner `_build_cmd(allowed)` (`llm.py:161-166`: `claude -p --output-format json --json-schema … --allowedTools … --strict-mcp-config [--model …]`); runs via `_run(stdin_prompt, allowed=…)` with prompt on **stdin** (ECC fix #11), jittered rate-limit retry loop, vision re-ask on unread frames. Unparseable envelope/`result` → `LlmSchemaError` (not generic `RuntimeError`). Returns `(parsed_dict, model_that_answered_or_pinned_fallback, frames_unread)`. Called by `responder._default_claude_model`.
- `claude_json(...)` — thin wrapper: `claude_json_meta(...)[0]` — the bare-dict contract preserved for callers that don't need provenance, notably `studio/actions.py` which binds `model = claude_json` for the manual "regenerate caption" Studio action (`llm.py:239-244`).
