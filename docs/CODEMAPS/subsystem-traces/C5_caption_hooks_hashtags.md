...
- `_MAX_TARGET_PICKS = 30` (module var) — the hard CEILING in the moment-pick **prompt** (never an ingest quota).
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure: computes the "up to N picks" ceiling shown to the LLM in `moment_pick_prompt` only. `ingest_moments` does NOT cap pick count — validity + within-owner overlap dedup are the only ingest gates (see [fresh-ingestion-trace.md](../fresh-ingestion-trace.md) §2). Called by `moment_pick_prompt`.
