...
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure: computes a prompt **CEILING** (not a quota) on how many non-overlapping clips to aim for, by source length and content-type band. `<=0` duration -> `0` (no target line in prompt, model decides freely); below the band floor -> `1` (one whole-source clip); else `round(duration / band.span)`, floored at 1, capped at `_MAX_TARGET_PICKS`. Called by `moment_pick_prompt`. (Fan-out is uncapped/model-driven in practice — validity + within-owner overlap dedup are the only hard limits downstream.)
...
- `LlmSchemaError(RuntimeError)` — class; MOL-227 — raised when `claude -p` output cannot be parsed into the requested JSON schema (unparseable envelope, non-object envelope, or non-JSON `result` after repair). The responder routes this to `_on_deterministic_fail` (labelled degrade + attempt ceiling), not a silent pending wedge.
- `_JSON_FENCE_RE`, `_json_candidates(text)`, `_extract_json_object(text)` — JSON-repair helpers (MOL-241): when `structured_output` is absent, `claude_json_meta` tries `json.loads(result)` first, then salvages prose-wrapped replies by scanning ```json fences and balanced `{…}` objects (`llm.py:29-78`, `:236-239`).
...
- `claude_json_meta(prompt: str, schema: dict, *, timeout: float = 300.0, images: list[str] | None = None, model: str | None = None) -> tuple[dict, str | None, bool]` — **THE ACTUAL LLM-CALLING PRIMITIVE.** … Prefers `structured_output` from the response envelope; on a string `result`, tries `json.loads` then `_extract_json_object` salvage (logs `JSON-repair` on success, raises `LlmSchemaError` if both fail). Returns `(parsed_dict, model_that_answered_or_pinned_fallback, frames_unread)`. Called by `responder._default_claude_model`.
