...
- `_target_pick_count(duration: float, band: Band = TALK) -> int` — pure: computes a **prompt-only ceiling** for how many clip picks to suggest (not enforced at ingest — `ingest_moments` has no pick-count cap). `<=0` duration -> `0`; below the band floor -> `1`; else `round(duration / band.span)`, floored at 1, capped at `_MAX_TARGET_PICKS`. Called by `moment_pick_prompt`.
