...
```
    ...
    publish-queue/map-media      reconcile/gc/resolve/unhold  autopilot/studio
            ...
            digest.render_*    (flock) → pipeline.advance /     daemon.install/status/
                                ...
                                amplify/retire / track.pull_     postiz_lifecycle.ensure_up /
             ...
             ┌──────────────────────────────────────────────────────────┐
             │ paused(cfg) -> log run/paused, heartbeat, do NO work      │
             ...
             │ → _learn_pass (live backend; amplify/retire own flags)    │
             │   (both FANOPS_LEARN_* OFF -> read-only, no ledger write) │
...
| `studio` | inline in `_dispatch` (`studio.app.create_app`) | Launch the local Flask cockpit web UI | May request Postiz via `postiz_lifecycle.ensure_up` then print `system_health` — **LIVE actions are reachable from within the UI** (Studio approve/publish flows are the human-gated live surface, outside this cluster) |
...
- `_learn_pass(cfg, *, window="30d")` — the extracted E1 learning pass (metrics fetch outside lock, classify + the two gated actuators inside one transaction). Raises on error; caller (`_cmd_run_pass`) catches and logs. **Side effects**: the network fetch and `pull_metrics`' metric write happen ALWAYS; the two state-changing actuators fire only when their own flag is on — `amplify` (ledger mutation + agent-request-file writes) needs `cfg.learn_amplify`, `retire` (clip/moment/unshipped-post suppression) needs `cfg.learn_retire`. Both default OFF, and neither is gated on `learning_validated`. With both OFF the pass logs `amplify_skipped` / `retire_skipped` and writes no lifecycle state. Called by `_cmd_run_pass` (`run` verb, entered only on `cfg.is_live_backend`).
...
- `cmd_map_media(cfg)` — READ-ONLY-w.r.t.-Instagram: mirrors live-only media (`reconcile.project_imported_media`), fills imported-row metrics (`track.pull_imported_insights`), saves ledger, prints counts. **Side effects**: Meta Graph GET calls, ledger save. Called by `_dispatch`.
...
### `health.py` — observe + snapshot strip writer (bring-up is NOT here)

- `DepHealth` (`NamedTuple`) — `(name, ok, detail)`, one dependency's red/green verdict (also used via `health_model`).
- `_docker_health()` — shells `docker info` (8s timeout); `False` if `docker` isn't on PATH or the call raises/times out (broad `except Exception`, deliberate — never let a health probe itself crash). Called by `system_health` / `health_model.dep_health_list`.
...
- `postiz_health(cfg)` / `zernio_health(cfg)` — live liveness helpers for `system_health`.
- `system_health(cfg)` — the live red/green list for Docker + Postiz + Zernio, in launch order. Called by `cli._dispatch` (`studio` verb).
- `_postiz_compose_dir()` — path resolver for observe (`health_model`); **not** a compose spawn.
- `refresh_runtime_snapshots(cfg)` — **named snapshot strip writer** (`deps_health.json` / `daemon_strip.json` / `strip_metrics.json` via `refresh_dep_snapshot` / `refresh_daemon_strip_snapshot` / `refresh_strip_metrics`). Callers today: `studio.app_routes_golive.do_golive_health` (write-on-read), `cli` `--loop` post-pass. Snapshot *readers* must not Call `refresh_*` / `write_json_atomic` / `postiz_health_probe` (CPDP-WP4 ratchet).
- **Infra bring-up owner:** sole `FunctionDef ensure_up` is `postiz_lifecycle.ensure_up` (ondemand script + `_should_autostart`). Call sites: `cli` studio launch, `post.run`, `reconcile`, `studio.actions`. Deleted from this module: `health.ensure_up` / `_start_docker` / `_start_postiz`.
...
- ~~`is_past_due(scheduled_time, now)`~~ — **REMOVED** from `src/` (Wave 3); live sibling is `is_due_or_past` below.
- `is_due_or_past(scheduled_time, now)` — pure, `<=` (mirrors `publish_due`'s gate semantics — "would fire on the next tick"); **unparseable → `True`** (treat-as-stale, the safe direction for the `go_live` readiness check — a torn time blocks the flip rather than silently passing). Called by `studio.golive.go_live`, `doctor.py`.
...
Steps 2-5 are the body of `cli._cmd_run_pass` (`cli.py:1109`); step 1 and step 6 sit in `_dispatch`'s `run` branch around it.

1. **Preflight gates** (`cli._dispatch`, `run` branch, `cli.py:1614-1615`): `_check_accounts(cfg)` then `_check_preflight(cfg)` — both READ-ONLY; either can abort the whole cycle with exit 2 before any mutation happens (bad `accounts.json`, or an `llm`/`postiz` config missing its binary/key).
2. **Operator brake** (`_cmd_run_pass`, `cli.py:1118-1120`): `pipeline_run.paused(cfg)` — the `00_control/paused` marker written by `fanops pause` — is checked BEFORE the run lease is taken, so a paused pump never even contends for the run flock and `fanops advance` by hand stays unblocked. It logs `run/paused` and returns `{"paused": True, "awaiting": {}}` rather than `None`: a DICT keeps the `--loop` heartbeat flowing (silence would freeze the recorded code SHA and make the keeper SIGTERM-kickstart the pump) and keeps the one-shot exit at 0, because a pause is not a failure.
3. **Respond+advance convergence loop** (`cli.py:1129-1140`, up to **10 iterations**):
   ...
   - **Error handling**: the entire iteration body is wrapped in `try/except Exception as e: print("run halted: ..."); return None` — the caller turns that `None` into exit 1, so **a single failing iteration DOES abort the whole `run` invocation** (not fail-open/continue). This is deliberate: the responder's LLM call or `publish_due`'s auth re-raise are treated as fatal-enough to stop rather than silently retry-loop. The `run` verb itself is one-shot per invocation; it is the **daemon's** cadence that provides the retry at the next scheduled fire, not an in-process retry.
4. **Stuck-gate reporting** (`cli.py:1144-1146`): if the loop exhausted 10 iterations still with an open gate, `_gates_blocked_note` prints a loud stderr line + logs `gates_blocked` — but this does NOT change the exit code (stays 0); it's a monitoring signal, not a failure.
5. **Post-loop passes** (`cli.py:1152-1248`), each independently gated and each in its **own try/except that swallows and logs, never propagates**:
   - `_learn_pass(cfg)` — entered on `cfg.is_live_backend`, but that predicate is TRANSPORT, not authorization: inside the pass, `amplify` fires only under `cfg.learn_amplify` (`FANOPS_LEARN_AMPLIFY`, default OFF) and `retire` only under `cfg.learn_retire` (`FANOPS_LEARN_RETIRE`, default OFF), and neither is gated on `learning_validated`. With both flags OFF the pass pulls metrics, classifies, logs `amplify_skipped` / `retire_skipped` (carrying `winners=` / `losers=`) and writes no lifecycle state. `AuthError` prints a distinct "learn skipped" line + logs `auth_error`; any other `Exception` logs `error` and is swallowed.
   ...
   - `expand_vocab_if_due` (hashtag vocab) — NOT gated on live backend; gated on `FANOPS_RESPONDER=llm` inside the helper and driven by a persona-fingerprint change, not a clock; own try/except; swallowed.
   - `refresh_store_if_due` (hashtags) — NOT gated on live backend (only on a scrape session, handled inside the helper), throttled by `last_complete_pass` rather than file mtime so a throttled write cannot buy silence; own try/except; swallowed. A corrupt-personas abort is logged loudly (`store_refresh_aborted`), distinct from a genuine refresh.
   - `refresh_account_stats_if_due` (IG follower snapshot) — throttled; own try/except; never raises.
   - `refresh_corpora_if_due` (persona corpora) — INPUT-DRIVEN off a `personas.json`+`hashtags.json` fingerprint, never a clock; own try/except; an abort is logged as `corpora_refresh_aborted`.
6. **Heartbeat** (`cli.py:1634` in the `--loop` path, `cli.py:1647` one-shot): `_heartbeat(cfg, s)` runs in `_dispatch`'s `run` branch, OUTSIDE `_cmd_run_pass`, on every pass that returned a summary — including a paused one, which is why the pause returns a dict. A halted pass (step 3's `None`) emits none. It prints a JSON line to stdout and appends to `run.log`; this is the sole liveness signal `daemon.status`'s staleness check reads.
7. **Exit**: 0 in the normal/paused/stuck-gate/swallowed-error cases; only step 3's per-iteration hard failure (a raised exception from responder or `advance` itself) returns exit 1.

**Conclusion on fail-open vs fail-closed**: the pipeline's per-unit stages (steps inside `advance()`) are individually quarantined (a bad clip/source doesn't halt others — traced in other clusters), and the post-loop passes are each independently fail-open (swallow+log, never crash the run). But the **respond+advance loop itself is NOT fail-open at the top level** — an uncaught exception from the responder or from `advance()` (e.g. a fatal auth error escaping `publish_due`) aborts the entire `run` invocation with exit 1. The **daemon** (launchd) is what makes the overall system resilient to this: each `StartInterval` fire is an independent, fresh `fanops run` process: one bad tick returns exit 1, but launchd fires the next tick at the same cadence regardless of the previous exit code (`RunAtLoad`/`StartInterval` don't gate on prior success) — so daemon-level continuity is provided by launchd's scheduling, not by in-process retry logic.
...
- `timeutil.is_past_due` — former strict-`<` variant; live sibling `is_due_or_past` remains (used by `studio.golive.go_live`, `doctor.py`).

**Call-graph blind spots (NOT dead — Jinja/argparse registration):**
- `src/fanops/timeutil.py` `to_local_display` / `to_local_input` — registered as Jinja filters in `studio/app.py`.
- `src/fanops/cli.py` `_http_url` — argparse `type=` callback.
...
- `src/fanops/health.py` — `_docker_health`: broad `except Exception`, documented ("FileNotFound / Timeout / OSError -> down, never raise"). Intentional. Bring-up failures live under `postiz_lifecycle.ensure_up`, not here.
