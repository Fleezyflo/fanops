...
- ~~`group_review_by_account_surface(rows)`~~ — **REMOVED** from `src/` (Wave 3); account-pivot rows render without this grouper.
...
- `group_review_by_batch(cards)` — **PURE-READ** (pure function). Groups editable cards by real Batch, unbatched group sorts last. Live Jinja filter (`app.jinja_env.filters`); used by `_review_body.html`.
...
- `src/fanops/studio/views_review.py` `provenance_chips` — Jinja GLOBAL (live; `app.jinja_env.globals`).
- ~~`group_review_by_account_surface`~~ — **REMOVED** from `src/` (Wave 3).
- `src/fanops/studio/views_review.py` `group_review_by_batch` — Jinja filter (live; used by `_review_body.html`).

Given the high proportion of these landing in `views_review.py`/`views_results.py` template-formatting helpers, most remaining zero-caller flags are genuine template-only consumers (Jinja) rather than truly dead code — but `zero_post_clips` / `metrics_stale_hint` still deserve a template grep before treating as confirmed live.
