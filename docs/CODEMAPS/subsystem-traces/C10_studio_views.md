...
- `gate_rows(cfg)` — **PURE-READ**. Enumerates pending agent gates (`moments`/`moment_hooks`/`captions`) via `agentstep.pending`. Called by `app.create_app` (feeds `gates.html`).
...
- `_cast_cause(led, post, affinities)` — **PURE-READ**. Names WHY an account got a moment from `Moment.affinities` (P13). Called by `_surface`.
...
- `account_lanes(led, accounts, cfg, *, source_id, now, state=None)` — **PURE-READ**. RF6 account-first lane view from `_affinity_index` → `Moment.affinities`. Called by `app_routes_review.register_review_routes`.
