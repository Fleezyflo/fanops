...
- `gate_rows(cfg)` — **PURE-READ**. Enumerates every pending agent gate (`moments`/`moment_hooks`/`captions` only — `moment_casting` removed P11) via `agentstep.pending`/`request_path` (`views.py:852`), reading each request JSON; a torn/unreadable request file is silently `continue`d (fail-open, matches its own docstring — the corruption is already logged elsewhere by `latest_request_id`). Called by `app.create_app` (feeds `gates.html`).
