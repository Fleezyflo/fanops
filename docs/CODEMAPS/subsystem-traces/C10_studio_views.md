...
- `_cast_cause(led, post, affinities)` — **PURE-READ**. Names WHY an account got a moment, from `Moment.affinities` (single-owner index via `_affinity_index`). Called by `_surface`.
...
- `account_lanes(led, accounts, cfg, *, source_id, now, state=None)` — **PURE-READ**. Builds the RF6 account-first lane view: `is_cast` from `Moment.affinities` via `_affinity_index` (not post existence). Called by `app_routes_review.register_review_routes`.
