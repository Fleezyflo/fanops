...
- `_cast_cause(led, post, affinities)` — **PURE-READ**. Names WHY an account got a moment, from the durable `AccountSelection` (falls back to legacy affinities tag only for pre-v9 sources). Called by `_surface`.
...
- `account_lanes(led, accounts, cfg, *, source_id, now, state=None)` — **PURE-READ**. Builds the RF6 account-first lane view: one lane per account showing every decided moment's cast/uncast state from the durable `AccountSelection` (not post existence). Called by `app_routes_review.register_review_routes`.
