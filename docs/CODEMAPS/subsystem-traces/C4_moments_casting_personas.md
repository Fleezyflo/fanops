...
- `Accounts.active()` — pure filter on `AccountStatus.active`. Called by `live_ready_channels`, `surfaces`, `validate`, `moments._pick_personas`, `crosspost.crosspost_clips`, Studio go-live flows.
