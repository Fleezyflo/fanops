...
> `hooks_by_persona`, `scoped_caption_surfaces`, and `casting_bias` are **gone**. Operator
...
Legacy note: the pre-P11 LLM casting stage (durable selection records,
...
- `ingest_moments` — validates picks, `_drop_overlaps` is **within-owner** (cross-owner overlap is allowed),
  ...
  No durable selection record, no `moment_casting` gate discard.
...
> The pre-P11 LLM casting symbols (the casting gate pair, `account_selection_admits`,
> `repair_casting_selections`, `casting_bias`, durable selection records/`SelectionFact`) were **deleted in
...
- **Removed (P11/MOL-152):** the LLM casting gate pair, `_stage_casting`, durable selection records/`SelectionFact`, `casting_bias`/`FANOPS_CASTING_BIAS`, `account_selection_admits`, `hooks_by_persona`.
