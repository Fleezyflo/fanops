...
> per-account selection records, `hooks_by_persona`, `scoped_caption_surfaces`, and `casting_bias` are **gone**. Operator
...
Legacy note: the pre-P11 LLM casting stage (casting gate verbs, durable per-account selection records,
...
- `ingest_moments(led, cfg, source_id)` — **reads a gate response**, validates + within-owner dedups picks, stamps `pick.personas[0]` → `Moment.affinities`, and reconciles into `picked` moments. On all-invalid picks → `SourceState.error`; on `[]` → non-terminal `moments_empty` (no reconcile). On reconcile: discards stale `moment_hooks` gates only (`discard_gates_for`); **no** `moment_casting` gate or durable selection discard (removed P11/MOL-152). Then `led.reconcile_moments` + `picks_decided`. Called by `pipeline._stage_ingest_moments`.
...
> The symbols below (casting gate request/ingest verbs, `account_selection_admits`,
> `repair_casting_selections`, `casting_bias`, durable per-account selection/`SelectionFact` models) were **deleted in
...
- **Removed (P11/MOL-152):** casting gate request/ingest verbs, `_stage_casting`, durable per-account selection/`SelectionFact`, `casting_bias`/`FANOPS_CASTING_BIAS`, `account_selection_admits`, `hooks_by_persona`.
