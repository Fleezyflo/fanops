...
- `Accounts.active()` — pure filter on `AccountStatus.active`. Called by `live_ready_channels`, `surfaces`, `validate`, `pipeline._owner_caption_surfaces`/`_stage_render_and_caption`, `crosspost.crosspost_clips`, `moments._pick_personas`/`_hook_personas_for_moment`, Studio views/actions throughout.
