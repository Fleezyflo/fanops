...
- `Persona` (pydantic `BaseModel`) — fields: `id`, `name`, `voice`, `hashtag_corpus`, `intake`, `content_focus`, `selection_scope`, `hook_angle`. Legacy `energy` is stripped at load (`personas.py:76-77`, MOL-170). Per-persona `clip_profile`/`framing` pins and freeform directive overrides were **retired in M3/M3e** — cut length/framing derive from `content_focus` via `derive_cut_spec`.
...
- `Accounts.active()` — pure filter on `AccountStatus.active`. Called by `live_ready_channels`, `surfaces`, `validate`, `moments._pick_personas`, `crosspost.crosspost_clips`.
...
- `Accounts.validate()` — pure: returns a list of config-problem strings — missing per-platform ids, the R2/D5/D15 drift state (one side of `integrations`/`backends` set without the other), duplicate handles, and malformed rows skipped at load (`skipped_rows`). Called by `cli._check_accounts`, `doctor.doctor_report`, `pipeline.advance`, Studio's `actions_run.run_advance`/`run_prepare`, `golive.go_live`.
