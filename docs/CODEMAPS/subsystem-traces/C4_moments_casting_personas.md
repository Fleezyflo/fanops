...
- `Accounts.active()` — pure filter on `AccountStatus.active`. Called by `live_ready_channels`, `surfaces`, `validate`, `pipeline._owner_caption_surfaces`, `crosspost.crosspost_clips`.
...
- `Accounts.validate()` — pure: returns a list of config-problem strings — missing per-platform ids, the R2/D5/D15 drift state (one side of `integrations`/`backends` set without the other), duplicate handles, and malformed rows surfaced via `skipped_rows`. Called by `cli._check_accounts`, `doctor.doctor_report`, `pipeline.advance` (persona/differentiation strings logged as `differentiation_warn`), Studio's `actions_run.run_advance`/`run_prepare`, `golive.go_live`.
