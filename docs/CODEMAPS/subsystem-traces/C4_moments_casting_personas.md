...
### `casting.py` — post-P11 affinity gate only (`affinity_admits`; archived pre-P11 inventory below)
...
- `Accounts.active()` — pure filter on `AccountStatus.active`. Called by `live_ready_channels`, `surfaces`, `validate`, `moments._pick_personas`, `crosspost.crosspost_clips`, `pipeline._aspects_for`, `pipeline._owner_caption_surfaces`, `batches.create_batch` (via `active_handles`), Studio views/actions throughout.
...
- `Accounts.validate()` — pure: returns a list of config-problem strings — missing per-platform ids, the R2/D5/D15 drift state (one side of `integrations`/`backends` set without the other), duplicate handles, and (when `cfg.account_casting` is ON and any active account has persona metadata) missing persona links / cut specs that match the global (no differentiation). Called by `cli._check_accounts`, `doctor.doctor_report`, `pipeline.advance`, Studio's `actions_run.run_advance`/`run_prepare`, `golive.go_live`.
