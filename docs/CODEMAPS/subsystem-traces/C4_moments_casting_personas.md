...
- `ingest_moments` — validates picks, `_drop_overlaps` is **within-owner** (cross-owner overlap is allowed),
  ...
  No durable selection table discard, no `moment_casting` gate discard.
...
> The symbols below (pre-P11 casting gate request/ingest, `account_selection_admits`,
> `repair_casting_selections`, `casting_bias`, durable selection table/`SelectionFact`) were **deleted in
...
- **Removed (P11/MOL-152):** pre-P11 casting gate request/ingest, `_stage_casting`, durable selection table/`SelectionFact`, `casting_bias`/`FANOPS_CASTING_BIAS`, `account_selection_admits`, `hooks_by_persona`.
