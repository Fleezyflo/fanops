...
- `Accounts.validate()` — pure: returns a list of config-problem strings — missing per-platform ids, the R2/D5/D15 drift state (one side of `integrations`/`backends` set without the other), duplicate handles, and (when `cfg.account_casting` is on) missing persona links / cut specs that match the global (shared-cut warning). Called by `cli._check_accounts`, `doctor.doctor_report`, `pipeline.advance`, Studio's `actions_run.run_advance`/`run_prepare`, `golive.go_live`.
