...
- `Accounts.active()` — pure filter on `AccountStatus.active`. Called by `live_ready_channels`, `surfaces`, `validate`, `crosspost.crosspost_clips`, `pipeline._owner_caption_surfaces`, `caption.request_captions`.
