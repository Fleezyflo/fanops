...
- `Accounts.active()` — pure filter on `AccountStatus.active`. Called by `live_ready_channels`, `surfaces`, `validate`, `moments._pick_personas`, `crosspost.crosspost_clips`, Studio go-live/read paths. (Pre-P11 casting helpers `_persona_donor_moments`/`casting_gate_failed_to_open` removed with the LLM casting stage.)
