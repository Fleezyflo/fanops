...
- `Accounts.active()` — pure filter on `AccountStatus.active`. Called by `live_ready_channels`, `surfaces`, `validate`, `moments._pick_personas`/`_hook_personas_for_moment`, `crosspost.crosspost_clips`.
