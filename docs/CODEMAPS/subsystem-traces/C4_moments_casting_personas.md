...
- `ingest_moments(led, cfg, source_id)` — **reads a gate response**, validates + dedups picks (within-owner overlap only), stamps `pick.personas[0]` → `Moment.affinities`, reconciles into `Moment` at state `picked`. On reconcile: discards stale `moment_hooks` gates only (`moments.py:365`) — no `moment_casting` gate, no `AccountSelection` drops (removed P11/MOL-152). Sets `SourceState.picks_decided`.
- `request_moment_hooks(led, cfg, source_id, accounts=None)` — **writes one gate request per `picked` moment**; `_hook_personas_for_moment` sends **only the owner** (P6). Called by `pipeline._stage_moment_hooks`.
- `ingest_moment_hooks(led, cfg, source_id, accounts=None)` — **atomic-per-source** ingest: writes `m.hook` only (no `hooks_by_persona`). Promotes to `decided`, sets `moments_decided`. Called by `pipeline._stage_moment_hooks`.

### `casting.py` — REMOVED P11/MOL-152 (was LLM casting stage + `account_selection_admits`)

The pre-P11 functions (`request_moment_casting`, `ingest_moment_casting`, `account_selection_admits`,
`repair_casting_selections`, `casting_gate_pending`, `_record_fact`, …) and `casting_bias.py` are **gone**.
Current file is 22 lines: `affinity_admits(cfg, moment, account)` only — see Post-P15 snapshot above.

### `casting_bias.py` — REMOVED P11

Module deleted with the LLM casting teardown. No `casting_reach_prior`, no `FANOPS_CASTING_BIAS` reader in `config.py`.
...
- `casting_directive(p)` — **REMOVED** (was casting-prompt compiler; retired with LLM casting stage P11). `selection_directive` / pick lenses replace it for the moments gate.
...
- `Accounts.active()` — pure filter on `AccountStatus.active`. Called by `live_ready_channels`, `surfaces`, `validate`, `moments._hook_personas_for_moment`.
...
## Owner routing (single-owner `Moment.affinities` — current model)

**As of P11/P15, there is NO LLM casting stage and NO durable `AccountSelection`.**

- **Pick attribution (P5):** `ingest_moments` stamps `pick.personas[0]` → `Moment.affinities` (single owner; `[]` = persona-blind → fan-to-all).
- **Gate:** `cfg.account_casting` (`config.py:593-599`) — DEFAULT ON. When OFF, `affinity_admits` admits every surface (ignores persisted affinities). When ON: `affinities==[]` → fan-to-all; non-empty → admit iff `account in affinities` (`casting.py:10-22`).
- **Caption scope (P10):** `pipeline._owner_caption_surfaces` uses the **same** `affinity_admits` predicate as crosspost — owner × platform, no `scoped_caption_surfaces`.
...
- **Removed (MOL-152):** `_stage_casting`, `request_moment_casting`/`ingest_moment_casting`, `account_selection_admits`, `casting_bias`, `hooks_by_persona`, `Post.variant_hook` per-account hook maps.
