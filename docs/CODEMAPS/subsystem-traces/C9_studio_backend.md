...
- `reburn_hook(cfg, post_id, hook, *, now=None)` (lines 176-212) — **ffmpeg subprocess, no LLM (P9)**.
  Updates `Moment.hook` and re-renders the shared clip via `render_moment` (no per-post `variant_hook` field).
  Runs render outside the lock; moment+clip write inside a short re-guarded transaction.
...
- `approve_with_hook(cfg, clip_id, *, now=None)` (lines 102-137) — restores `moment.hook` from
  `hook_removed`, lock-free warm via `_warm_hooked_render`, re-renders via `render_moment`, **rolls back**
  the whole transaction if render errors or `hook_burn_failed`, then approves every `awaiting_approval`
  post of the clip.
...
- `set_per_account_hooks`, `set_account_casting`, `set_ai_responder`, `set_clip_profile` (lines
  219-296) — each a single `_dual_write` of one named env var. `set_per_account_hooks` dual-writes
  `FANOPS_CREATIVE_VARIATION` but there is no `cfg.creative_variation` property (`config.py` has no
  `getenv` — documentation-only; views hardcode `creative_variation=False`). None are publish-affecting
  switches orthogonal to `FANOPS_LIVE`.
...
- `preview_media_path(cfg, led, post_id)` (lines 8-27) — **pure read + lock-free path resolution (P9)**.
  Ladder: (1) `post.render_id` → `Render.path` if on disk; (2) local `post.media_urls[0]` if present;
  (3) base `clip.path`. Owner-moment hook is already burned on the shared clip at render — no on-demand
  per-post re-burn. Fail-open `except Exception: pass` removed (straight returns only).
