...
- `reburn_hook(cfg, post_id, hook, *, now=None)` (lines 176-212) — **ffmpeg subprocess, no LLM** (P9).
  Re-burns the **owner-moment** hook: updates `m.hook`, re-renders the shared clip via `render_moment`
  (no `cfg.creative_variation` gate — that flag is documentation-only in `config.py`; differentiation
  is intrinsic when `account_casting` is ON). `hook_burn_failed` surfaces as `ok=True, detail.hook_burned=False`.
...
- `approve_with_hook(cfg, clip_id, *, now=None)` (lines 102-137) — restores `moment.hook` from
  `hook_removed`, lock-free pre-warms via `_warm_hooked_render`, re-renders inside the transaction,
  **rolls back** on render error or `hook_burn_failed`, then approves every `awaiting_approval` post
  on the clip. (No `cfg.creative_variation` gate — removed with P9 owner-moment hook model.)
...
- `preview_media_path(cfg, led, post_id)` (lines 8-27) — **pure read** (lock-free). Resolution ladder: (1) `post.render_id` → existing `Render.path` on disk; (2) `post.media_urls[0]` if a local `file://` or bare path exists; (3) base `clip.path`. No on-demand ffmpeg burn — owner hooks are burned at `render_moment` (P9). All exception paths are fail-open (returns `None`, never crashes).
