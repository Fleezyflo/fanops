...
- `reburn_hook(cfg, post_id, hook, *, now=None)` (lines 176-220) — **P9 owner-moment re-burn**: updates `Moment.hook`, re-renders the shared clip via `render_moment` (ffmpeg only, no LLM). No `cfg.creative_variation` gate — differentiation is intrinsic when `account_casting` is ON. Rolls back on render error; surfaces `hook_burn_failed` as `ok=True, detail.hook_burned=False`.
...
- `approve_with_hook(cfg, clip_id, *, now=None)` (lines 102-138) — restores `moment.hook` from `hook_removed`, lock-free warm via `_warm_hooked_render`, then re-renders inside the transaction (`render_moment`); **rolls back the whole transaction** on render error or `hook_burn_failed`. Approves every `awaiting_approval` post of the clip. (No `creative_variation` refuse — P9 owner-moment model.)
...
- `preview_media_path(cfg, led, post_id)` (lines 8-27) — **pure read, no on-demand ffmpeg** (P9). Resolution ladder: (1) `post.render_id` → existing `Render.path` if on disk; (2) `post.media_urls[0]` if a local `file://` or bare path exists; (3) base `clip.path`. Owner-moment hook is already burned at `render_moment` — preview serves the shared clip file.
