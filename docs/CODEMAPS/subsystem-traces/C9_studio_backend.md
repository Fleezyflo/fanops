...
- `reburn_hook(cfg, post_id, hook, *, now=None)` (lines 176-241) — **ffmpeg subprocess, no LLM**.
  P9 owner-moment re-burn: updates `m.hook` and re-renders the shared clip via `render_moment` (no
  `cfg.creative_variation` gate — that property was removed; per-account differentiation is intrinsic
  when `account_casting` is ON). Runs outside the lock; transaction adopts the warm render.
...
- `approve_with_hook(cfg, clip_id, *, now=None)` (lines 102-138) — restores `moment.hook` from
  `hook_removed`, re-renders via `render_moment` (rolls back the transaction on render error or
  `hook_burn_failed`), then approves every `awaiting_approval` post of the clip. No
  `creative_variation` gate (removed — owner-moment hook is always the burn source under P9).
...
- `set_per_account_hooks`, `set_account_casting`, `set_ai_responder`, `set_clip_profile` (lines
  219-296) — each a single `_dual_write` of one named env var. `FANOPS_CREATIVE_VARIATION` is
  dual-written by `set_per_account_hooks` but has **no `getenv` in `config.py`** (documentation-only
  runtime flag; Studio hardcodes `creative_variation=False` in views). `set_account_casting` toggles
  `FANOPS_ACCOUNT_CASTING` (DEFAULT ON). None of these are publish-affecting switches orthogonal to
  `FANOPS_LIVE`.
...
- `preview_media_path(cfg, led, post_id)` (lines 8-27) — **pure read**. Resolution ladder:
  (1) `post.render_id` → existing `Render.path` if on disk; (2) `post.media_urls[0]` local file;
  (3) base `clip.path`. Owner-moment hook is burned at `render_moment` — no on-demand ffmpeg render
  and no `cfg.creative_variation` gate (removed). Fail-open on parse/path errors.
