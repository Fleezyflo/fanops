...
- `reburn_hook(cfg, post_id, hook, *, now=None)` (lines 176-241) — **ffmpeg subprocess, no LLM**.
  Updates the owner-moment's `m.hook` and re-renders the shared clip via `render_moment` (P9 — no
  `cfg.creative_variation` gate; that env var has no `getenv` in `config.py`). Computes the
  content-addressed render id via `account_render_spec`, attempts an account-specific re-cut
  (`render_account_cut`) or falls back to `overlay.burn_hook_only` — both run *outside* the lock;
  the `Render` entity is added/pointed-at inside a short re-guarded transaction. `hook_burn_failed`
  is surfaced as `ok=True, detail.hook_burned=False` (a warning, not a rollback, since this is an
  edit not an approve).
...
- `approve_with_hook(cfg, clip_id, *, now=None)` (lines 102-138, `actions_approve.py`) — restores
  `moment.hook` from `hook_removed`, lock-free pre-warms via `_warm_hooked_render`, then in one
  transaction re-renders (fingerprint-skip adopts the warm), **rolls back the whole transaction** if
  the render errors or if `rc.hook_burn_failed`, and approves every `awaiting_approval` post of the
  clip. No `cfg.creative_variation` gate (removed with per-post variant hooks).
...
- `preview_media_path(cfg, led, post_id)` (lines 8-27) — **pure read + lock-free path resolution**.
  Resolution ladder: (1) `post.render_id` → existing `Render.path` if on disk; (2) `post.media_urls[0]`
  when it is a local `file://` or bare path; (3) base `clip.path`. Serves the owner-moment clip
  (hook burned at `render_moment`). No on-demand ffmpeg render, no `cfg.creative_variation` gate.
