...
- `normalize_account_handle(handle) -> str` (models.py:445-448) — strips whitespace and leading `@`, lowercase. Pure. Called by ledger migration helpers and account validation.
- `validate_account_handle(handle) -> str` (models.py:451+) — raises `ValueError` on invalid handle shape. Called by account writers.
- `stitch_plan_id(clip_id, asset_ids, strategy_key, plan_params) -> str` (models.py:482+) — content-addressed id. Pure.
- `batch_id(name, created_at) -> str` (models.py:505+) — `content_id("batch", name, created_at)`. Pure.
...
**File format (v11):** one JSON document at `00_control/ledger.json` with `{"schema_version": int, "sources": {}, "moments": {}, "clips": {}, "posts": {}, "tag_log": {}, "variant_streaks": {}, "stitch_plans": {}, "batches": {}, "renders": {}, "imported_media": {}}`. (`account_selections` / `selection_facts` dropped at v11 — see `_migrate_v11_drop_selection_maps`.)
