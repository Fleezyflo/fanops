...
> dropped on load via `_migrate_v10_drop_selections` (`ledger.py:178`). Crosspost routing now reads
...
- `normalize_account_handle(handle) -> str` — strips whitespace and leading `@`. Pure. Called by ledger migration helpers (`_account_selection_id`, v8→v9 lift only).
...
- `_migrate(raw, from_version) -> dict` (ledger.py:243+) — hop-chains through `_MIGRATIONS` dict; raises `ControlFileError` on a chain gap.
- `_MIGRATIONS` dict (ledger.py:209+) — version N ← transform table, versions 1–10.
...
**File format:** one JSON document at `00_control/ledger.json` with `{"schema_version": int, "sources": {}, "moments": {}, "clips": {}, "posts": {}, "tag_log": {}, "variant_streaks": {}, "stitch_plans": {}, "batches": {}, "renders": {}, "imported_media": {}}`. (Legacy v7–v10 ledgers may carry transient `selection_facts` / `account_selections` maps until the v10→v11 hop drops them on load.)
