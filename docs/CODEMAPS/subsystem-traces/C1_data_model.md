...
- **Removed v11 (P12/MOL-154):** `add_account_selection`, `account_selection_for`, `selections_of_source`, `drop_account_selection`, `add_selection_fact`, `get_selection_fact`, `moment_ids_selected_for`, `cast_handles_for`, `_prune_orphan_selection_ids` — durable selection maps dropped; routing reads `Moment.affinities` via `casting.affinity_admits` only. Migration helpers `_migrate_v8_account_selections` / `_migrate_v11_drop_selection_maps` remain for old ledgers.
...
- Queries (ledger.py:530-548): `already_seen`, `sources_in_state`, `clips_in_state`, `posts_in_state`, `moments_of`, `clips_of`, `posts_of`, `posts_of_account` — O(n) scans.
- `reconcile_moments(source_id, keep)` (ledger.py:555+) — upsert+cascade-delete core.
...
**File format:** one JSON document at `00_control/ledger.json` with `{"schema_version": int, "sources": {}, "moments": {}, "clips": {}, "posts": {}, "tag_log": {}, "variant_streaks": {}, "stitch_plans": {}, "batches": {}, "renders": {}, "imported_media": {}}`. (`selection_facts` / `account_selections` were dropped at v11 — see `_migrate_v11_drop_selection_maps`.)
