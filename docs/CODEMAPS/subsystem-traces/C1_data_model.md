...
- `normalize_account_handle(handle) -> str` (models.py:445+) — strips whitespace and leading `@`. Pure. Called by ledger migration helpers (`ledger._account_selection_id` during v8→v9 lift only).
...
- Idempotent adds (ledger.py:485-491): `add_source`, `add_moment`, `add_clip`, `add_post`, `add_render`, `get_render`, `add_imported_media`, `get_imported_media`.
- `add_stitch_plan`, `approve_stitch_plan`, `dismiss_stitch_plan` (ledger.py:665-671) — structural-hooks spine.
- `add_batch`, `get_batch`, `batches_for_account` (ledger.py:677-681) — Account-First batch registry.
...
**File format:** one JSON document at `00_control/ledger.json` with `{"schema_version": int, "sources": {}, "moments": {}, "clips": {}, "posts": {}, "tag_log": {}, "variant_streaks": {}, "stitch_plans": {}, "batches": {}, "renders": {}, "imported_media": {}}`. (`account_selections`/`selection_facts` were dropped at SCHEMA v11 — migration-only, not persisted.)
