...
- `normalize_account_handle(handle) -> str` (models.py:445+) — strips whitespace and leading `@`. Pure. Called by ledger migration helpers (`_migrate_v8_account_selections`, v8→v9 hop only).
...
- **Removed v11 (P12/MOL-154):** `add_account_selection`, `account_selection_for`, `selections_of_source`, `drop_account_selection`, `add_selection_fact`, `get_selection_fact`, `moment_ids_selected_for`, `cast_handles_for`, `_prune_orphan_selection_ids` — crosspost routing reads `Moment.affinities` via `casting.affinity_admits` only.
...
**File format (SCHEMA_VERSION=11):** one JSON document at `00_control/ledger.json` with `{"schema_version": int, "sources": {}, "moments": {}, "clips": {}, "posts": {}, "tag_log": {}, "variant_streaks": {}, "stitch_plans": {}, "batches": {}, "renders": {}, "imported_media": {}}`. (`account_selections` / `selection_facts` maps dropped at v11 — v8→v9 lift migration helpers remain for upgrade hops only.)
