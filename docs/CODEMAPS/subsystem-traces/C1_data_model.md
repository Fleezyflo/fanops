...
- `normalize_account_handle(handle) -> str` (models.py:445-448) — strips whitespace and leading `@`. Pure. Called by ledger migration helpers.
...
- ~~`add_account_selection` / `selections_of_source` / `cast_handles_for`~~ — **REMOVED v11** (P12/MOL-154); crosspost reads `Moment.affinities` via `casting.affinity_admits` only.
...
**File format:** one JSON document at `00_control/ledger.json` with `{"schema_version": int, "sources": {}, "moments": {}, "clips": {}, "posts": {}, "tag_log": {}, "variant_streaks": {}, "stitch_plans": {}, "batches": {}, "renders": {}, "imported_media": {}}`. (`account_selections` / `selection_facts` dropped at v11 — migration-only on load.)
