...
**Functions:**
- `is_real_submission_id(sid) -> bool` (models.py:389-398) — returns False if `sid` is falsy or starts with `"fanops_"`. Called by `track.pull_metrics` / status logic.
- `normalize_account_handle(handle) -> str` (models.py:445-448) — strips whitespace and leading `@`, lowercases. Pure. Called by legacy migration helpers in `ledger.py` (`_account_selection_id` during v8→v9 lift only).
- `validate_account_handle(handle) -> str` (models.py:451-458) — strict write-boundary canonicalizer; raises `ValueError` on blank/illegal charset.
- `stitch_plan_id(clip_id, asset_ids, strategy_key, plan_params) -> str` (models.py:482-488) — content-addressed id. Pure.
- `batch_id(name, created_at) -> str` (models.py:505-509) — `content_id("batch", name, created_at)`. Pure.
...
> **Removed v11 (P12/MOL-154):** `add_account_selection`, `account_selection_for`, `selections_of_source`, `drop_account_selection`, `add_selection_fact`, `get_selection_fact`, `selection_facts_of_*`, `moment_ids_selected_for`, `cast_handles_for`, `_prune_orphan_selection_ids` — the durable selection maps and their query helpers are gone; crosspost routing reads `Moment.affinities` via `casting.affinity_admits` only.
...
**File format:** one JSON document at `00_control/ledger.json` with `{"schema_version": int, "sources": {}, "moments": {}, "clips": {}, "posts": {}, "tag_log": {}, "variant_streaks": {}, "stitch_plans": {}, "batches": {}, "renders": {}, "imported_media": {}}`. (`selection_facts` / `account_selections` were dropped at SCHEMA_VERSION 11.)
