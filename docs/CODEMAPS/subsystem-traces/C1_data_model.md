...
### `SelectionFact` / per-account selection model — **REMOVED v11 (P12/MOL-154)**
...
```
(Moment.affinities is the crosspost gate input — no separate legacy selection table post-v11.)
...
### Per-account selection method sum-type — **REMOVED v11** (historical; was a validated discriminator on the deleted selection model)
