...
- `normalize_account_handle(handle) -> str` (models.py:445-449) — strips whitespace and leading `@`. Pure. Called by ledger migration helpers.
...
- Queries (ledger.py:530-555): `already_seen`, `sources_in_state`, `clips_in_state`, `posts_in_state`, `moments_of`, `clips_of`, `posts_of`, `posts_of_account` — O(n) scans.
