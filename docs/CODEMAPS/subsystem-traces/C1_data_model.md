...
- `render_id: Optional[str]` → Render.id (optional pointer; owner hook burns onto shared clip at `render_moment`)
...
- ~~`add_account_selection` / `add_selection_fact` / `selections_of_source` / `selection_facts_of_*`~~ — **REMOVED v11** (P12/MOL-154); crosspost reads `Moment.affinities` via `casting.affinity_admits` only. v8→v9 lift (`_migrate_v8_account_selections`) and v10→v11 drop (`_migrate_v11_drop_selection_maps`) remain for old ledgers.
...
- `_prune_orphan_selection_ids(source_id)` — **REMOVED v11** (was post-reconcile selection cleanup for durable `AccountSelection` maps).
...
**File format:** one JSON document at `00_control/ledger.json` with `{"schema_version": int, "sources": {}, "moments": {}, "clips": {}, "posts": {}, "tag_log": {}, "variant_streaks": {}, "stitch_plans": {}, "batches": {}, "renders": {}, "imported_media": {}}`. (`selection_facts` / `account_selections` dropped at SCHEMA_VERSION 11.)
