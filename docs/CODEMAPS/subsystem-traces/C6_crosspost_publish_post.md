...
**Class `AwaitingCounts(TypedDict)`** — `{moments, moment_hooks, moment_casting, captions}`. No methods.
...
- `_default_claude_model(kind, payload, *, cfg=None, log=None) -> dict` — the production model function: builds the gate's JSON schema + prompt, calls `claude_json_meta` (in `llm.py`, outside this cluster) pinned to `cfg.llm_model_for(kind)`, attaches `frames` images for the vision gates (`moments`, `moment_hooks`, `moment_casting`), emits a provenance log line (`model`, `prompt_sha`, `brief_sha`), and stamps `hook_frames_unread` onto a degraded `moment_hooks` response. Called by `LlmResponder.__init__` (as the default `_model`).
...
Module constants: `_SCHEMA` (kind → pydantic model), `_PROMPT` (kind → prompt-builder function), `_VISION_GATES = ("moments", "moment_hooks", "moment_casting")`.
