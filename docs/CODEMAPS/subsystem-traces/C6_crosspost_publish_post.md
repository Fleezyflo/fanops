...
1. **Birth: `None → awaiting_approval`** — `crosspost.py:228-240` (`_mint_surface_post`, inside `led.add_post(Post(... state=PostState.awaiting_approval ...))`). This is the ONLY place in the entire 17-file set that constructs a brand-new `Post`.
...
**Class `AwaitingCounts(TypedDict)`** — `{moments, moment_hooks, captions}` (mirrors `GATE_KINDS`; `moment_casting` removed P11). No methods.
...
- `_default_claude_model(kind, payload, *, cfg=None, log=None) -> dict` — builds schema + prompt, calls `claude_json_meta`, attaches `frames` for vision gates (`moments`, `moment_hooks` only). Called by `LlmResponder.__init__`.
...
Module constants: `_SCHEMA` (kind → pydantic model), `_PROMPT` (kind → prompt-builder; three kinds: moments/moment_hooks/captions), `_VISION_GATES = ("moments", "moment_hooks")`.
...
- `PostizPoster._youtube_title(self, post, led=None) -> str` — reads the owner-moment `m.hook` via clip→moment lineage (`postiz.py:353-362`); floors to `cfg.artist_name` if too short/empty. **No** `Post.variant_hook` field (removed P11). Called by `publish`.
...
- `_materialize_variant_media(led, cfg, post, accts) -> None` — **no-op stub** (`run.py:173-175`, P9): owner-moment hook is burned on the shared clip at `render_moment`; per-post materialize removed with the per-account hook fork. Called by `_ensure_media` (retained call site for forward-compat).
