...
- `PostizPoster._youtube_title(self, post, led=None) -> str` — reads the owner-moment hook via `led.moments[clip.parent_id].hook` (NOT `post.variant_hook` — removed); floors to `cfg.artist_name` if too short/empty. Called by `publish`.
...
- `_materialize_variant_media(led, cfg, post, accts) -> None` — **P9 no-op stub** (`run.py:173-175`): owner-moment hook is burned on the shared clip at `render_moment`; there is no per-post variant materialize path. Kept as a call-site anchor in `_ensure_media`. (`Post.variant_hook` removed.)
