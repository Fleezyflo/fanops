...
1. **Birth: `None → awaiting_approval`** — `crosspost.py:228-232` (`_mint_surface_post`, inside `led.add_post(Post(... state=PostState.awaiting_approval ...))`). This is the ONLY place in the entire 17-file set that constructs a brand-new `Post`.
...
- `screen_model_text(obj)` — MOL-166: ONE text-screen chokepoint; sanitizes model-authored strings before `*.response.json` is written (`responder.py:23-41`). Called by `_answer_one`.
- `_gate_source_id(led, kind, key) -> str | None` — resolves the owning source id for a gate key (`moments`/`moment_hooks` directly; `captions` via clip→moment→source). Pure. Called by `_terminate_gate_source`, `_mark_gate_degraded`.
...
- `LlmResponder._answer_one(self, cfg, kind, model_cls, key, log) -> bool` — **the per-gate answer body**: reads the pending request JSON, captures `rid_before` (TOCTOU guard, AUDIT A3), calls the model (retrying once on `LlmTimeoutError`), re-verifies `rid_after == rid_before` (drops a stale answer if the gate was re-seeded mid-call), self-stamps the authoritative `request_id`, validates via `model_cls(**out)`, runs `screen_model_text`, and writes the response atomically. On `LlmContextLimitError` → `_mark_gate_degraded`. On `ValidationError` or `LlmSchemaError` → `_on_deterministic_fail` (degraded_reason + per-gate attempt counter; MOL-235 ceiling promotes the owning source to `SourceState.error` after `_GATE_DETERMINISTIC_MAX=3` failures). Transient `Exception` → log + stay pending. Thread-safe by construction (all state is per-key local). Called by `answer_pending`.
- `LlmResponder._on_deterministic_fail(...)` — MOL-235: stamps `degraded_reason` and bumps the per-gate attempt counter via `agentstep.bump_attempts`; on ceiling calls `_terminate_gate_source`. Called by `_answer_one`.
- `LlmResponder._terminate_gate_source(...)` — MOL-235 ceiling: promotes the owning source to `SourceState.error` + `error_reason`. Load+save OUTSIDE the flock. Called by `_on_deterministic_fail`.
- `LlmResponder._mark_gate_degraded(...)` — AGENT-2: best-effort `degraded_reason` stamp on the gate's owning source (load+save outside flock). Called by `_on_deterministic_fail`, `_mark_context_limit`.
- `LlmResponder._mark_context_limit(self, cfg, kind, key, reason) -> None` — back-compat shim: delegates to `_mark_gate_degraded` with a context-limit prefix. Called by `_answer_one`.
- `LlmResponder.answer_pending(self, cfg) -> int` — snapshots every pending `(kind, model_cls, key)` tuple serially, then either runs sequentially (`cfg.concurrent_sources` OFF, default) or fans out over a `ThreadPoolExecutor(max_workers=cfg.concurrent_workers)`. Returns the count of fresh answers written. Called per call_graph by `cli.py`/`daemon.py` (outside this cluster).
...
Module constants: `_SCHEMA` (kind → pydantic model), `_PROMPT` (kind → prompt-builder function), `_VISION_GATES = ("moments", "moment_hooks")` (`responder.py:48`), `_GATE_DETERMINISTIC_MAX = 3` (`responder.py:49`).
