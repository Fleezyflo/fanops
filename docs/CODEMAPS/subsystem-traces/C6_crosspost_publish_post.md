...
- `LlmResponder._mark_gate_degraded(self, cfg, kind, key, reason) -> None` — best-effort: resolves the gate's owning source (`moments`/`moment_hooks` keys → source id prefix; `captions` keys → clip→moment→source) and stamps a visible `degraded_reason` on it (`responder.py:137-156`). Loads+saves the ledger OUTSIDE the advance() flock. Wrapped in its own outer try/except (a load/save failure must not crash the responder). Called by `_mark_context_limit`.
- `LlmResponder._mark_context_limit(self, cfg, kind, key, reason) -> None` — back-compat shim wrapping `_mark_gate_degraded` with the legacy context-limit prefix (`responder.py:161-163`). Called by `_answer_one`.
...
- `PostizPoster._youtube_title(self, post, led=None) -> str` — resolves the owner-moment hook via `led.moments[clip.parent_id].hook` (P9: `Post.variant_hook` removed) as the YouTube title, floored to `cfg.artist_name` if too short/empty (`postiz.py:353-362`). Called by `publish`.
...
- `_materialize_variant_media(led, cfg, post, accts) -> None` — **P9 no-op stub** (`run.py:178-180`): owner-moment hooks are burned on the shared clip at `render_moment`; per-post materialize was removed with `Post.variant_hook`. Still called by `_ensure_media` for signature stability.
