...
- `PostizPoster._youtube_title(self, post, led=None) -> str` — resolves the owner-moment hook via `led`→clip→moment (`m.hook`), floored to `cfg.artist_name` if too short/empty. Called by `publish`.
...
- `_materialize_variant_media(led, cfg, post, accts) -> None` — **no-op stub (P9)**: owner-moment hook is burned onto the shared clip at `render_moment` / `render_account_cut`; publish reads `post.render_id` or the clip path directly. Retained as a call-site anchor in `_ensure_media` for signature stability. Called by `_ensure_media`.
