...
- `LlmResponder._mark_context_limit(self, cfg, kind, key, reason) -> None` — back-compat shim: wraps `_mark_gate_degraded` with a context-limit prefix (`responder.py:161-163`). The owning source is resolved by `_mark_gate_degraded` for `moments`/`moment_hooks` keys directly, via clip→moment→source for `captions` keys. (`moment_casting` removed P11.) Called by `_answer_one`.
