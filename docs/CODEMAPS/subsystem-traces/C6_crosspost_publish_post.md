...
- `PostizPoster._youtube_title(self, post, led=None) -> str` — reads the owner-moment hook via `clip→moment→m.hook` (P9 — `Post.variant_hook` field removed); floors to `cfg.artist_name` if too short/empty. Called by `publish`.
...
- `_materialize_variant_media(led, cfg, post, accts) -> None` — **P9 no-op** (`post/run.py:173-175`): owner-moment hook is burned on the shared clip at `render_moment`; no per-post materialize path remains. Called by `_ensure_media` (call preserved for seam stability).
