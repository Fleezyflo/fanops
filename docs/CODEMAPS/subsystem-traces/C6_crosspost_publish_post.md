...
- `write_request(cfg, *, kind, key, payload) -> str` — mints a fresh `request_id` (hash of kind/key/prev-id/payload), atomically writes (temp + `os.replace`, same-filesystem asserted), deletes any stale response file, and **`clear_attempts`** (MOL-231 — a re-seed resets the responder retry counter). **Side effect**: disk write, unlinks the response. Called widely across moments/caption producer modules (outside this cluster).
...
- `discard_gate(cfg, kind, key) -> None` — unlinks both request+response files **and `clear_attempts`** (MOL-231), idempotent. Called outside this cluster (amplify/re-pick flows).
...
- `_attempts_path`, `bump_attempts`, `clear_attempts` — per-gate retry counter files (`*.attempts.json`); `clear_attempts` is invoked from `write_request` and `discard_gate` so a fresh request or explicit discard never inherits stale attempt counts.
