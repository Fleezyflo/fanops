...
- `PostizPoster._youtube_title(self, post, led=None) -> str` — reads the owner-moment hook via `clip→moment→m.hook` (`postiz.py:353-362`); floors to `cfg.artist_name` if too short/empty. Called by `publish`.
...
- `_materialize_variant_media(led, cfg, post, accts) -> None` — **no-op stub (P9)**: owner-moment hooks burn on the shared clip at `render_moment`/`render_account_cut`; there is no per-post variant materialize path. Called by `_ensure_media` (returns immediately).
