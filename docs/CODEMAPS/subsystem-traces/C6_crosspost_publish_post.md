...
> `repair_casting_selections`, casting-gate defer, `hooks_by_persona`, or persisted `Post.variant_hook`
> (owner hook lives on `Moment.hook`; Studio mirrors it as `variant_hook` in view rows only).
...
- `PostizPoster._youtube_title(self, post, led=None) -> str` — reads the owner-moment hook via `led.moments[clip.parent_id].hook` (not a `Post` field); floors to `cfg.artist_name` when absent/too short. Called by `publish`.
...
- `_materialize_variant_media(led, cfg, post, accts) -> None` — **P9 no-op stub** (`run.py:173-175`): owner-moment hook is burned on the shared clip at `render_moment`; there is no per-post publish-time materialize path. Called by `_ensure_media` (call retained for seam stability).
