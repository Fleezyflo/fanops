...
1. **Birth: `None → awaiting_approval`** — `crosspost.py:228-295` (`_mint_surface_post`, inside `led.add_post(Post(... state=PostState.awaiting_approval ...))`). This is the ONLY place in the entire 17-file set that constructs a brand-new `Post`.
