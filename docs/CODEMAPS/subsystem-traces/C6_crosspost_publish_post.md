...
- `LlmResponder._mark_context_limit(self, cfg, kind, key, reason) -> None` — back-compat shim wrapping `_mark_gate_degraded` with a context-limit prefix; resolves the gate's owning source (directly for `moments`/`moment_hooks` keys, via clip→moment→source for `captions` keys). (`moment_casting` keys removed P11.)
