...
- `LlmResponder._mark_context_limit(self, cfg, kind, key, reason) -> None` — back-compat shim wrapping `_mark_gate_degraded` with a context-limit prefix. Called by `_answer_one`.
