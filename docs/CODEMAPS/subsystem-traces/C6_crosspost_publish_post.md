...
> `repair_casting_selections`, casting-gate defer, or `hooks_by_persona`. (`Post.variant_hook` field removed P9 — hook lives on `Moment.hook`; Studio surfaces it as a view-model `variant_hook`.)
...
- `PostizPoster._youtube_title(self, post, led=None) -> str` — walks clip→moment and uses `m.hook` as the YouTube title (`postiz.py:353-362`), floored to `cfg.artist_name` if too short/empty. Called by `publish`.
...
- `_materialize_variant_media(led, cfg, post, accts) -> None` — **P9 no-op stub** (`run.py:173-175`): owner-moment hook is burned on the shared clip at `render_moment`; there is no per-post variant materialize path. Called by `_ensure_media` (call preserved for API stability).
