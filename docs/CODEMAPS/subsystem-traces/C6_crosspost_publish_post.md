...
- `account_render_spec(cfg, *, clip, hook, acct) -> tuple[render_id, wants_cut, profile, top_bias]` — **RENAMED P9:** now `render_spec(cfg, *, clip, hook, moment)` (`crosspost.py:84-94`) — pure, content-addressed owner-moment Render identity + cut decision (`wants_cut = bool(hook)`). Called by `render_moment_file`.
- `render_account_file(...)` — **RENAMED P9:** now `render_moment_file(led, cfg, *, post, target_clip, src, caller="approve") -> RenderPlan` (`crosspost.py:113-135`) — burns `m.hook` onto disk (per-account cut via `render_account_cut` or shared-clip `overlay.burn_hook_only` fallback). PURE render-to-disk, NO ledger mutation. Called by Studio approve warm paths (`actions_approve.render_moment`); NOT called at publish (`_materialize_variant_media` is a P9 no-op).
...
- `screen_model_text(obj)` — MOL-166 text-screen chokepoint: sanitizes model-authored strings before `*.response.json` is written. Called by `LlmResponder._answer_one`.
- `_gate_source_id(led, kind, key) -> str | None` — resolves the owning source id for a gate key (`moments`/`moment_hooks` key on source id directly; `captions` keys on clip→moment→source). Pure. Called by `_mark_gate_degraded`, `_terminate_gate_source`.
...
- `LlmResponder._answer_one(self, cfg, kind, model_cls, key, log) -> bool` — **the per-gate answer body**: reads the pending request JSON, captures `rid_before` (TOCTOU guard, AUDIT A3), calls the model (retrying once on `LlmTimeoutError`), re-verifies `rid_after == rid_before` (drops a stale answer if the gate was re-seeded mid-call), validates via `model_cls(**out)`, screens text via `screen_model_text`, writes the response atomically, and `clear_attempts` on success (MOL-236). Catches `LlmContextLimitError` → `_mark_context_limit` (visible `degraded_reason`); `ValidationError` / `LlmSchemaError` → `_on_deterministic_fail` (degraded + attempt counter, MOL-226/MOL-227); generic `Exception` → log + stay pending (transient). After `_GATE_DETERMINISTIC_MAX` (3) deterministic failures on the same gate, `_terminate_gate_source` promotes the owning source to `SourceState.error` (MOL-235). Thread-safe by construction (all state is per-key local). Called by `answer_pending`.
- `LlmResponder._on_deterministic_fail(self, cfg, kind, key, reason, log) -> None` — stamps `degraded_reason` via `_mark_gate_degraded`, bumps the per-gate attempt counter (`bump_attempts`), and terminates the source on ceiling. Called by `_answer_one`.
- `LlmResponder._terminate_gate_source(self, cfg, kind, key, reason) -> None` — MOL-235 ceiling: stamps the owning source `SourceState.error` + `error_reason`, load+save outside the flock. Called by `_on_deterministic_fail`.
- `LlmResponder._mark_gate_degraded(self, cfg, kind, key, reason) -> None` — best-effort: resolves the gate's owning source via `_gate_source_id` and stamps a visible `degraded_reason`. Loads+saves the ledger OUTSIDE the advance() flock. Called by `_on_deterministic_fail`, `_mark_context_limit`.
- `LlmResponder._mark_context_limit(self, cfg, kind, key, reason) -> None` — back-compat shim wrapping `_mark_gate_degraded` with a context-limit prefix. (`moment_casting` keys removed P11.) Called by `_answer_one`.
...
Module constants: `_SCHEMA` (kind → pydantic model), `_PROMPT` (kind → prompt-builder function), `_VISION_GATES = ("moments", "moment_hooks")` (`responder.py:46-48`), `_GATE_DETERMINISTIC_MAX = 3` (`responder.py:49`, MOL-235).
...
- `bump_attempts(cfg, kind, key) -> int` — MOL-235: increments the per-gate deterministic-failure counter on disk; returns the new count. Called by `LlmResponder._on_deterministic_fail`.
- `clear_attempts(cfg, kind, key) -> None` — MOL-236: resets the per-gate attempt counter on a successful answer. Called by `LlmResponder._answer_one`.
...
- `PostizPoster._youtube_title(self, post, led=None) -> str` — reads the owner-moment hook from `led.moments[clip.parent_id].hook` (not `Post.variant_hook` — field removed P9); floors to `cfg.artist_name` when absent/too short. Called by `publish`.
...
- `_materialize_variant_media(led, cfg, post, accts) -> None` — **P9 no-op stub** (`run.py:178-180`): owner hook is burned onto the shared clip at `render_moment` during the pipeline pass; publish reads `clip.path` / pre-stamped `media_urls`. Historical per-post variant materialize removed with `Post.variant_hook`.
...
- `_ensure_media(led, cfg, post, backend, *, account_id=None) -> None` — resolves `post.media_urls` to network-fetchable URLs: calls the no-op `_materialize_variant_media`, applies shrink if the backend has an upload cap, then either uploads the clip fresh (`ensure_clip_media`) or uploads a pre-stamped `file://` render (`ensure_render_media`) for rows that still carry a per-render path. Called by `_publish_one`.
