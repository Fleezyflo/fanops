...
- `_archive_published(cfg, post) -> None` — writes a day-bucketed, human-browsable JSON record to `06_published/<YYYY-MM-DD>/<post_id>.json` (0600, replacement-atomic via `controlio.write_text_atomic` — mkstemp + `os.replace`, so an interrupted re-archive keeps the prior record; MOL-728 retired the `O_CREAT|O_WRONLY|O_TRUNC` final-path open). **Fully fail-open**: any exception is caught, logged, and re-caught if even the logging fails — the archive is a convenience artifact, never a publish blocker. Called by `_publish_one` (only on a confirmed `PostState.published` final state, OUTSIDE the finalize transaction so an archive failure can never roll back the just-committed publish).
...
- `_request_id(post) -> str` — the `x-request-id` for one create attempt: `uuid5(_ZERNIO_REQ_NS, _request_name(post))`, where `_request_name` is the **canonical JSON array** `json.dumps(["1", post.id, created_at, platform, account_id], ensure_ascii=False, separators=(",", ":"))` — NOT a delimiter join, which would alias (report 11 §15 C-2, corrected before any deployment). Stable per **record incarnation × platform × resolved Zernio account**, so every retry of one attempt replays instead of creating a second post. NOT `post.id` alone — `crosspost` pops a `failed`/`rejected` record and remints it under the identical `post.id` with a fresh `created_at`, and `run.py` refreshes `account_id` at publish (report 11 §8/D6). Pure. Called by `ZernioPoster._create`/`publish`.
- `_require_request_identity(post) -> TerminalFailure | None` — refuses to POST when `created_at`/`account_id`/`platform` is absent, so a missing discriminator costs **zero network calls** and is never defaulted (a fabricated one would make two incarnations share an id). Pure. Called first by `ZernioPoster._create`.
- `_extract_zernio_id(body) -> str | None` — accepts `_id`/`id`/`postId` aliases + a nested `post.{...}` shape. Pure. Called by `_parse_create_body`.
- `_parse_create_body(body) -> ZernioCreateResult` — classifies a 2xx into `Created` / `IdempotentReplay` (HTTP 200 + `existingPost`) / `ReconciliationRequired`. **Never** returns `TerminalFailure`: a 2xx means Zernio accepted something, so an unreadable shape is ambiguous, not provably-not-accepted. Pure. Called by `ZernioPoster._create`.
- `_extract_409_candidate(body) -> str | None` — the 409's `details.existingPostId`; malformed → `None`, never a raise. Pure. Called by `ZernioPoster._create`.
- `_retry_after_s(resp) -> float | None` — `Retry-After` in seconds; absent/HTTP-date → `None` (caller falls back to bounded backoff). Pure. Called by `ZernioPoster._create`.
- `_fits_deadline(started, wait) -> bool` — whether the NEXT send still lands inside `_RETRY_DEADLINE_S` (240s), on `time.monotonic()`, strictly inside Zernio's ~5-min `_IDEMPOTENCY_WINDOW_S` (300s). Pure. Called by `ZernioPoster._create`.
...
- `build_zernio_payload(*, account_id, platform, content, media_urls, scheduled_time) -> dict` — builds the create-post body with `publishNow: true` (FanOps owns the schedule via its own `queued`/due-gate, so Zernio is never handed a future time). The `x-request-id` is a **header**, deliberately not built here: it is derived from the record's identity, not its content. Pure. Called by `ZernioPoster._create`.
- `_scrub_signed(s) -> str` / `_evidence(cfg, resp) -> str` / `_scrubbed_transport(exc, stage, cfg=None) -> BaseException` / `_put_signed(upload_url, path, ctype)` — the presign-upload helpers (bounded redacted evidence incl. `Allow`; signed-URL leak containment re-raising the SAME exception class). Called by `zernio_upload_media`.
- `zernio_upload_media(cfg, path, *, account_id=None) -> str` — **the supported programmatic upload contract** (OpenAPI v1.0.4): (1) POST `/media/presign` `{filename, contentType, size}` → `{uploadUrl, publicUrl, …}`; (2) **PUT `<uploadUrl>`** raw bytes, matching `Content-Type`, **no `Authorization`** (the URL carries the signature); (3) the caller puts `publicUrl` in `mediaItems[]`. Shrinks via `maybe_shrink_for_cap` first if oversize, raising `RuntimeError` if still over cap. 401 on presign → `ZernioAuthError` (a 401 on the signed PUT is an upload failure, not a key problem). `account_id` is retained for call-site compatibility and **unused** — presign is account-agnostic. Supersedes the `/media/upload-token` + `/media/upload` pair, which is documented for end-user workflows, was used programmatically, began answering 405 on 2026-07-16 and is deliberately **not** kept as a fallback (report 09 §6/§8.6). Called by `providers._zernio_uploader` (lazy factory — same false-positive flag as the Postiz uploader).
...
- `ZernioPoster._create(self, post) -> ZernioCreateResult` — **the actual publish network call** (report 11). Refuses pre-send without a request identity; sends `x-request-id` on **every** attempt, so a retry is a REPLAY, not a second post. `RequestException` (not `ConnectTimeout`) → `ReconciliationRequired` (body may have landed); `ConnectTimeout` → retried inside the deadline, then `TerminalFailure` if **nothing was sent**, else `ReconciliationRequired`; 200/201 → `_parse_create_body`; 401 → `ZernioAuthError` (fatal, the only raise); **409 → `ReconciliationRequired` + `details.existingPostId` as an unproven candidate — never `failed`**; 5xx → `ReconciliationRequired`; 429 → retried only while `Retry-After` fits the deadline, else `ReconciliationRequired` (the request reached Zernio, so the create may already have landed); other 4xx → `TerminalFailure` with the body **withheld** (this reason feeds `is_transient_failure_reason`'s substring scan, so an echoed "timeout"/"(503)" would flip a terminal into a re-queue loop). Called by `publish`.
- `ZernioPoster.publish(self, led, post_id) -> Ledger` — **protocol-conformant and unchanged in signature** (`Poster.publish`, shared with postiz/dryrun — the typed result never crosses it). The SOLE site mapping a create result onto the ledger: `Created`/`IdempotentReplay` → `submitted` + `submission_id` (identical state; the replay additionally logs a `idempotent_replay` audit event; `public_url` stays `safe_public_url(None) or post.public_url` — a placeholder mirroring Postiz's "no permalink at publish time" pattern, the real permalink comes later from `ZernioStatusClient`); `ReconciliationRequired` → `needs_reconcile` + `reconcile_candidate_id` (**never** `submission_id`) + the candidate mirrored into `error_reason` for downgrade visibility; `TerminalFailure` → `failed`. Called by `post/run.py:_publish_one`.

**Classes**: `ZernioAccount(NamedTuple)` — `id, name, platform`, no methods.

### post/zernio_outcome.py — the private Zernio create result (report 11 §4)

The typed result of ONE Zernio create attempt. **PRIVATE to the Zernio backend**: it never crosses the `Poster` protocol, which stays `publish(led, post_id) -> Ledger` for every backend — `postiz.py`, `dryrun.py` and `post/__init__.py` neither import nor know about it (invariant I-11). A type rather than inline state mutation so the 409 rule ("never `failed`") and the candidate rule ("never `submission_id`") each have exactly ONE owner: `ZernioPoster.publish`.

**Classes** (all `@dataclass(frozen=True, slots=True)`, no methods): `Created(post_id)` — a new post. `IdempotentReplay(post_id)` — Zernio recognised our `x-request-id` and returned the original; the SAME logical submission. `ReconciliationRequired(reason, evidence, candidate_post_id=None)` — disposition unknown; never success, never terminal. `TerminalFailure(reason, evidence)` — provably not accepted; the only result safe to mark `failed` (which is re-queueable). `ZernioCreateResult` is their union.
