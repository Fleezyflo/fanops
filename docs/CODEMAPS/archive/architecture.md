...
| Asset memory (M1) | the `origin_kind` axis (native vs third_party) end-to-end: models.Source.origin_kind + SourceState.{discovered,retired}; ingest `stage_inbox_candidates`/`ingest_staged` spine (origin_kind/inbox threading, sha-conflict WARN, write-once); config.thirdparty_inbox (peer staging dir); pipeline source-loop inert guard; ledger.retire_source (cascade, file kept) + rebuild_catalog (disk↔ledger reconcile); studio.asset_catalog + save_thirdparty_uploads/run_ingest_thirdparty + /library tab |
