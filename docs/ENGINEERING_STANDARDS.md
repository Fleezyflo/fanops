...
- **Enforcement owner:** authors + review; the no-extras contract is proven by CI. · **Current:** `partially-enforced` — the *contract* (a base install imports and refuses loudly) is proven by `scripts/base_install_smoke.py` via control **`CI-BASEINSTALL`**, which is **advisory**: it runs on every PR and is read, but does not block a merge. The *layout rule itself* is `documented-only`. · **Planned:** nothing. The staged promotion to a required context was cancelled with OGD on 2026-07-22; making it blocking again is an ADR-0101 amendment, not queued work.
