...
**Goal:** When a per-account hook variant has earned a *sustained, well-evidenced* win, automatically amplify its source (reopen → mine new moments) carrying the winning hook into the request — so proven winners get more reach — such that **no wrong signal can ever delete or unpublish LIVE content** (published/analyzed posts + their clips are structurally preserved by the C1-fixed cascade, which v3 does not change, and v3 itself never retires/deletes — AST-proven). v3 adds a *harder-gated trigger* to the existing `adjust.amplify` path; the amplify it triggers carries that path's pre-existing reconcile semantics (the already-posted winning *moment* is retired with its live post preserved; non-live sibling lineage is reconciled away — exactly as the v1/v2 learn-loop already does on weaker evidence). It does **not** introduce a new deletion path.
