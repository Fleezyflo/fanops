...
| `ACT-03` | RCDR:85-86 falsified `[OBS]` claim not retracted | **UNTOUCHED** — prompt §2 forbids documentation corrections |
| `ACT-04`/`ACT-05` | Track A (S1–S5) never applied; nothing visually accepted | **UNTOUCHED** — prompt §2 forbids applying Smart Reframing. **Newly established: `FANOPS_SMART_FRAMING` is not `0`** → S1–S5 are code-active, so report 05's finding stands (§5.1). |
