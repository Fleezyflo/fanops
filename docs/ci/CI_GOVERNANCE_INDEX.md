...
Six phases: **Investigation → Architecture → Governance → Implementation → Operational Governance
Deployment (OGD) → Program Closeout.** The first four are engineering (produce the policy + machinery);
OGD deploys the policy to live security settings; Closeout records and freezes. Authoritative status:
`CI_PROGRAM_LIFECYCLE.md`.
...
7. `CI_BRANCH_PROTECTION_MUTATIONS.md` — the OGD runbook (Phase 5) + the closeout/DNA specs.
8. *(after OGD)* `CI_PROGRAM_CLOSEOUT.md`, then `CI_GOVERNANCE_DNA.md` — the immutable records.
...
| 2 | **ADRs 0100–0102** | `docs/adr/010{0,1,2}-*.md` | The governing decisions | ✅ accepted (in principle) |
...
| 5 | **OGD Runbook** | `docs/ci/CI_BRANCH_PROTECTION_MUTATIONS.md` | Deployment procedure (Phase 5) | ✅ staged, not executed |
...
| OGD runbook + live branch protection | **operator** (applies) · ci-lane (authors) | operator action; policy ⇒ ADR |
...
1. Classify the change with the **decision tree** (§8): is it ADR-gated, normal-PR, or OGD?
...
4. **OGD** changes (live branch protection) follow the runbook: pre-image, one-at-a-time, re-probe, rollback.
...
| **Add / promote a required check** | Amend ADR-0101's required set → add to `intended_required_contexts` → mirror the workflow job name (DC-1) → deploy as a new **OGD** mutation. Never code-only. | **ADR** + OGD |
...
| **Change branch protection** | **OGD** procedure (runbook): capture pre-image, apply one mutation, re-probe, keep the rollback. A policy shift also amends ADR-0101/0102. | operator + (ADR if policy) |
...
graph TD
  ...
  subgraph P5["OGD (Phase 5, operator)"]
...
- **Phases 1–4 (Investigation, Architecture, Governance, Implementation): ✅ COMPLETE and merged.**
  Re-provable: `python -m tools.ci reconcile` (static clean; deployed = planned transition) and
  `python -m tools.ci selftest` (all negative controls fire).
- **Phase 5 (OGD): ⏳ not started** — operator-applied; runbook + rollback staged.
- **Phase 6 (Closeout + DNA, then freeze): ⏳ not started** — produced after OGD.

## 11 · Freeze declaration

When Phase 5 (OGD) is deployed **and** Phase 6 has produced both `CI_PROGRAM_CLOSEOUT.md` and
`CI_GOVERNANCE_DNA.md`, **this CI governance program is frozen and immutable.** The DNA document, the
