...
- **Unit tests:** 211 passed, 5 deselected. Check: `source .venv/bin/activate && python -m pytest -q -m "not integration"`.
...
- **Module/test parity:** 32 src modules (incl. `reconcile.py` for H4), 35 test files. The integration *marker* now selects 4 tests across 2 files (`tests/integration/test_e2e_real.py` ×2 incl. the amplify→real-render case, `test_blotato_smoke.py`, `test_publish_track_loop.py`) PLUS `tests/test_adjust.py::test_amplify_then_ingest_then_render_produces_new_clip` (marked `integration` because it shells real ffmpeg — CI-1). Unit-suite additions: `test_e2e_transcript_assertion.py` (CI-2 vocoder guard), `test_ledger_lock.py` H6, `test_reconcile.py` H4. Check: `ls src/fanops/*.py src/fanops/post/*.py | wc -l && ls tests/*.py tests/integration/*.py | wc -l` (32 / 35) and `python -m pytest -q -m integration --collect-only | tail -1`.
...
- **CI:** `.github/workflows/ci.yml` — unit job + an e2e job that installs the toolchain and runs the integration suite with `FANOPS_REQUIRE_E2E=1` (turns a tooling-absent skip into a failure, audit H10). HAS RUN on GitHub (Actions enabled). First two runs went RED on both jobs — two test-hygiene defects (CI-1 ffmpeg-in-unit, CI-2 espeak-vs-`slept`), both fixed this session; confirm the post-fix run is green. Check: `gh run list --repo Fleezyflo/fanops --limit 3` and `gh run view <id> --repo Fleezyflo/fanops`.
