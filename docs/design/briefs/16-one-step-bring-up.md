...
| B | **Docker + Postiz stack** | Found stack `Exited`; `docker compose up -d`; Postiz API returned **HTTP 502 for >2 min** | `postiz-ondemand.sh:53 ensure()` (in the separate `~/postiz-selfhost/` checkout) — *already* does Docker-up + idempotent up + honest 200/401 probe | **The Mastra crash-loop self-heal is missing** — see C |
| C | **Postiz `mastra_ai_spans` crash-loop** | Diagnosed `MastraError: tables can have at most 1600 columns`; brought up postgres alone; verified table empty + no FK dependents; `DROP TABLE mastra_ai_spans CASCADE`; full `up -d`; confirmed backend `↺0` + API `200` | Detected but **punts to a human** at `postiz-ondemand.sh:64-68` (in the separate `~/postiz-selfhost/` checkout) → `docs/POSTIZ_OPS.md §4` | The one true gap. §4 is a documented *manual* runbook for a *recurring* fault (POSTIZ_OPS.md §6: "until Postiz ships a fix, §4 is the workaround") |
