...
| **Hashtag scrape login** | `fanops hashtags scrape-login` → `cmd_hashtags_scrape_login` opens `00_control/scrape_chrome/<user>/` on a FanOps-owned CDP port (9331–9399, never 9222/9223), waits for a sessionid in THAT Chrome, then best-effort promotes the device envelope (`dump_settings` — the only envelope write). Lock scrape (`ig_web_scrape`) `fetch()`es inside that Chrome. Layer A remesure still uses `open_client(allow_reauth=False)`. Never `login()`, never system Chrome. | A web login is not an instagrapi session. Cookie export into Python is rejected by Instagram (login redirect). The FanOps Chrome page is the scrape client. |
