...
| **Hashtag scrape login** | `fanops hashtags scrape-login` → `cmd_hashtags_scrape_login` is the SOLE `open_client(allow_reauth=True)` call site. The unattended tick opens `allow_reauth=False`: search-probe the dump, Chrome-inject if `LoginRequired`, never `login()`. | A cold password login is an interactive/checkpoint act; the daemon may only reuse an already-logged-in Chrome session. |
