...
| **Hashtag scrape login** | `fanops hashtags scrape-login` → `cmd_hashtags_scrape_login` / `open_client(allow_reauth=True)`: instagrapi password login (cold-start or LoginRequired restore via `_clear_auth_keep_device`), then promotes the device envelope. Lock produce and remesure load the envelope via `open_client` (instagrapi — not `ig_web_scrape.open_web_session` / Safari XHR). No browser window. | Google Chrome is the operator's daily / DevTools browser. A FanOps Chrome instance hijacks the Dock. Lock/remesure use the on-disk instagrapi envelope. |
