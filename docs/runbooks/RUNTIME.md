...
| ~~`FANOPS_CREATIVE_VARIATION`~~ | — | **REMOVED** — not a live flag. `Config.creative_variation` does not exist; `tests/test_no_ghosts.py` bans the name. Setting the env var does nothing. Today one owner-moment hook burns at crosspost. See [docs/FLAGS.md](../FLAGS.md). |
...
| **Hashtag scrape login** | `fanops hashtags scrape-login` → `cmd_hashtags_scrape_login` is the SOLE `open_client(allow_reauth=True)` call site. It ensures `00_control/scrape_chrome/<user>/` (that Instagram account only), logs the Chrome argv for a normal login into THAT profile, then promotes the device envelope (`dump_settings` — the only envelope write). The unattended tick opens `allow_reauth=False`: load envelope + inject that profile's `sessionid`, search-probe, write nothing. Never `login()`, never system Chrome / DevTools. | A cold password login is an interactive/checkpoint act; the daemon is a reader of the FanOps profile, not a harvest of the operator's daily Chrome. |
...
- **(j) Per-account creative variation — v1 DELETED + v2 DONE (caption-bias loop).**
  **v1** (`FANOPS_CREATIVE_VARIATION`): **REMOVED** — not a live flag (see [docs/FLAGS.md](../FLAGS.md) /
  `tests/test_no_ghosts.py`). Setting the env var does nothing. Today one owner-moment hook burns at
  crosspost; per-account **cut** (length/framing) is `Account.clip_profile`/`framing`, never this flag.
