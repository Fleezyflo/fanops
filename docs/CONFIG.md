...
| Var | Default | Effect | Set |
|---|---|---|---|
| `FANOPS_CLIP_PROFILE` | `talk` | Leftover env name on disk. Clip length is the picker's start/end. Render copies that window (EOF clamp). This value is not a cut command | S |
...
| `FANOPS_IG_SCRAPE_USER` | None | Instagram username(s) for hashtag scrape. Comma-separated membership + LRU tiebreak. Lock picker is LRU + freeze-skip, Safari (no envelope json). Harvest remesure still needs a session file. Unattended tick: one tag, `delay_range` on each XHR | .env |
...
| `FANOPS_HASHTAG_SCRAPE_TRY_CAP` | 25 | Max tags per **injected/manual** scrape client this pass. The unattended tick walks **one tag** (instagrapi: spread work; `delay_range` paces each request). Incomplete remesure does not advance `last_complete_pass`. Lock unfinished does not stamp `researched_at` | .env |
...
| `FANOPS_HASHTAG_SCRAPE_DELAY` | `1,3` | instagrapi `delay_range`: random sleep **after each request** (`Client.delay_range = [1, 3]`). Same pair on live Safari `_json`. `"lo,hi"` seconds. `0` disables. Malformed keeps `1,3`. Live `_json` also +1 `accounts[user].used` per XHR (not per tag) | .env |
