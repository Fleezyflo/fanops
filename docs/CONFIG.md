...
| `FANOPS_IG_SCRAPE_USER` | None | Instagram username(s) for hashtag Layer A (instagrapi). Comma-separated list order = who measures first in a pass; each qualifying user (session on disk, not frozen, under day budget) may spend up to `FANOPS_HASHTAG_SCRAPE_TRY_CAP` attempts in that pass (MOL-857/900) | .env |
