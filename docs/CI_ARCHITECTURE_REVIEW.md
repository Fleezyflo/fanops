...
- **Anchor verification (immediately pre-delivery):** the `_CLI_PRINT_COUNT` budget as declared by `tests/test_internal_prints_routed.py` (this doc references it symbolically — it never copies the literal)
