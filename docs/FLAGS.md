...
| `intro_tease` | `FANOPS_INTRO_TEASE=1` | OFF | M6 structural-hooks: the intro-tease PRODUCER (LLM-vision matcher pairs a clean clip with an intro asset, compose-prepends a "wait for it" tease). Needs `hook_router` on (gates are always answered by the LLM); off → no matcher gate, no renders (non-regression) — [config.py:494](../src/fanops/config.py). Firewall: [test_router.py](../tests/test_router.py), [test_intro_match.py](../tests/test_intro_match.py) |
