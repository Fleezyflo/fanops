...
def preprocess_for_ocr(
    ...
    """Contrast/threshold preprocessing for purple-neon studio frames (ffmpeg filters only)."""
    src = Path(image_path)
    out = Path(output_path)
    out.parent.mkdir(parents=True, exist_ok=True)
    # Grayscale + mild contrast lift — whole-frame OCR on purple light is garbage; the crop + this
    # chain is tuned for white Arabic/English hook text on saturated magenta backgrounds.
