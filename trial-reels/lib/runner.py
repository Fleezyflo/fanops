...
def run_clip(
    ...
    *,
    ...
    hooks = list(recipes.get("hooks") or [])
    if not hooks and desk_result.get("cards"):
        hooks = [card["hook"] for card in desk_result["cards"]]
    stacks = list(recipes.get("stacks") or [])
    rehooks_s = tuple(recipes.get("rehooks_s") or (3, 8))
    cards_by_hook = {card["hook"]: card for card in desk_result.get("cards") or []}
    ...
    for hook in hooks:
        ...
        for stack in stacks:
            if not stack_gate_passes(stack, cut_length):
                result.variants.append(
                    VariantResult(
                        hook=hook,
                        stack=stack,
                        output_path=out_dir / f"{clip_id}_{hook}_{stack}.mp4",
            ...
            ass_path = work_dir / f"{clip_id}_{hook}_{stack}.ass"
            ...
            out_path = out_dir / f"{clip_id}_{hook}_{stack}.mp4"
            try:
                render_variant(
                    source=source,
                    output=out_path,
