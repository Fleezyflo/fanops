...
def run_clip(
    ...
    *,
    ...
    rendered_all = len(outputs) == len(plans) == TARGET_VARIANTS
    success = score.success and validation["ok"] and rendered_all
    message = score.message
    if validation["ok"] and not rendered_all:
        message = (
            f"desk write ok but only {len(outputs)}/{TARGET_VARIANTS} variants rendered"
