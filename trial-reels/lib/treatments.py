...
English: stitch whisper fragments into full sentences, then one treatment per sentence
(no clause crumbs or sliding windows). Arabic: one treatment per Whisper line with
global strict-substring drop (عزبتني inside لك كفاية عزبتني). Ships exactly
MAX_TREATMENTS distinct on-screen texts or fails closed.
"""
...
def _candidate_spans(sentence_tokens: list[_Token], language: str) -> list[tuple[int, int, bool]]:
    """Full grammatical unit only — one span per stitched sentence or whisper line."""
    del language
