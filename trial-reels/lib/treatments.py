...
English: clause-boundary spans inside stitched sentences, up to 20 non-nested picks.
Arabic: one treatment per Whisper line — honest ceiling when the transcript is thin.
"""
...
_EN_SPLIT_MARKERS = (" that ", " by ", " to ", " and ", " the ", " of ", " in ")
...
def _is_crumb(text: str, language: str, *, tokens: list[_Token], at_clause_boundary: bool = False) -> bool:
    ...
    if language == "en":
        ...
        if (
            first in _WEAK_HOOK_STARTERS_EN
            and not at_clause_boundary
            and not text.lower().startswith("so the next chapter")
        ):
            return True
...
def _candidate_spans(sentence_tokens: list[_Token], language: str) -> list[tuple[int, int, bool]]:
    ...
    for start in boundaries:
        for length in range(_MIN_HOOK_WORDS_EN, min(11, len(words) - start + 1)):
...
def _select_treatments(candidates: list[HookTreatment], language: str) -> list[HookTreatment]:
    """Pick up to MAX_TREATMENTS non-nested spans; pack densely, then prefer kind diversity."""
    by_line: dict[int, list[HookTreatment]] = {}
    ...
    for line_index in sorted(by_line):
        intervals = sorted(
            by_line[line_index],
            key=lambda item: (item.word_end - item.word_start, item.word_start),
        )
    ...
    selected: list[HookTreatment] = []
    remaining = list(packed)
    while remaining and len(selected) < MAX_TREATMENTS:
        kinds_seen = {item.kind for item in selected}
        best_index = 0
        best_key: tuple[float, int] = (float("-inf"), 0)
        for index, candidate in enumerate(remaining):
            kind_bonus = 1.0 if candidate.kind not in kinds_seen else 0.0
            words = len(candidate.text.split())
            length_score = 1.0 if 5 <= words <= 12 else 0.0
            key = (kind_bonus + length_score, -index)
            if key > best_key:
                best_key = key
                best_index = index
        selected.append(remaining.pop(best_index))

    selected.sort(key=lambda item: (item.source_order, item.word_start))
    return selected[:MAX_TREATMENTS]
