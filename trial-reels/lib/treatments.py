...
_EN_INTERNAL_STARTERS = frozenset(
    {
        "creates", "defines", "brings", "evaporates", "ability", "boss", "genuine", "respect",
        "corporate", "industry", "illusion", "layer", "moves", "execute", "that", "the", "modern",
        "missing", "reality", "padded", "recording", "booth", "powerful", "rare", "streets",
        "genuine", "respect", "illusion", "second", "completely", "required",
    }
)
...
# Major clause boundaries inside one English sentence — comma + subordinator only.
_EN_CLAUSE_MARKERS = (
    " that ",
    " which ",
    " by ",
...
def _starts_grammatically(text: str, *, word_start: int = 0) -> bool:
    words = [part for part in text.split() if part.strip()]
    if not words:
        return False
    ...
    if word_start == 0:
        ...
        if first in _WEAK_HOOK_STARTERS_EN:
            return False
        return True
    if first in _EN_INTERNAL_STARTERS:
        return _ends_sentence(last) and len(words) >= 5
    if first in {"by", "to", "of"}:
        return _ends_sentence(last) and len(words) >= 5
    if first in _EN_RELATIVE_STARTERS and _ends_sentence(last):
        return len(words) >= 4
    if first in _WEAK_HOOK_STARTERS_EN:
        return False
    return True
...
def _ends_at_major_boundary(text: str, line_text: str) -> bool:
    """True when *text* ends where a comma or subordinator split begins in *line_text*."""
    hook = text.strip()
    if not hook or hook not in line_text:
        return False
    pos = line_text.find(hook)
    if pos < 0:
        return False
    tail = line_text[pos + len(hook) :].lstrip()
    if not tail:
        return True
    return tail.lower().startswith(("by ", "that ", "which ", ","))


def _ends_grammatically(text: str, *, require_terminal: bool = False, line_text: str = "") -> bool:
    words = [part for part in text.split() if part.strip()]
    if not words:
        return False
    last = words[-1].rstrip("\"')]}")
    if last.endswith(",") or _ends_sentence(last):
        return True
    if require_terminal:
        if line_text and _ends_at_major_boundary(text, line_text):
            return True
        return False
    ...
    if language == "en":
        ...
        if is_full_whisper_line and _content_word_count(text, language) >= _MIN_CONTENT_WORDS_EN:
            if _ends_sentence(words[-1]) or words[-1].rstrip().endswith(","):
                return False
        if not _starts_grammatically(text, word_start=word_start):
            return True
        if not _ends_grammatically(
            text,
            require_terminal=not is_full_whisper_line,
            line_text=line_text or (tokens[0].line_text if tokens else ""),
        ):
            return True
...
def _marker_split_indices(line_text: str, words: list[str]) -> list[int]:
    ...
    for marker in _EN_CLAUSE_MARKERS:
        search_from = 0
        while True:
...
def _candidate_spans(sentence_tokens: list[_Token], language: str) -> list[tuple[int, int, bool]]:
    ...
    def add(start: int, end: int) -> None:
        if end <= start or (start, end) in seen:
            return
        seen.add((start, end))
        spans.append((start, end, start in set(boundaries)))

    # Full sentence and comma/subordinator sibling clauses only — no sliding windows.
    add(0, len(sentence_tokens))
    for index in range(len(boundaries) - 1):
        add(boundaries[index], boundaries[index + 1])
...
def _stitched_sentence_groups(transcript: dict[str, Any], language: str) -> list[list[_Token]]:
    """Stitched grammatical sentences for English hook enumeration."""
...
def _enumeration_groups(transcript: dict[str, Any], language: str) -> list[list[_Token]]:
    """Hook sources: stitched sentences + attested whisper lines (English), lines only (Arabic)."""
    if language == "ar":
        return _whisper_line_groups(transcript, language)

    groups: list[list[_Token]] = []
    seen_lines: set[str] = set()
    for sentence_tokens in _stitched_sentence_groups(transcript, language):
        line_text = sentence_tokens[0].line_text
        if line_text not in seen_lines:
            seen_lines.add(line_text)
            groups.append(sentence_tokens)
    for line_tokens in _whisper_line_groups(transcript, language):
        line_text = line_tokens[0].line_text
        if line_text not in seen_lines:
            seen_lines.add(line_text)
...
def _drop_parent_spans(treatments: list[HookTreatment]) -> list[HookTreatment]:
    """Per cite line, keep leaf spans — drop strict super-spans (nested substring farm)."""
    by_line: dict[str, list[HookTreatment]] = {}
    for item in treatments:
        by_line.setdefault(item.line_text, []).append(item)

    kept: list[HookTreatment] = []
    for line_text in sorted(by_line, key=lambda line: by_line[line][0].source_order):
        line_items = by_line[line_text]
        texts = [item.text for item in line_items]
        for item in line_items:
            if any(
                item.text != other
                and is_nested_hook_text(other, item.text)
                for other in texts
            ):
                continue
            kept.append(item)
...
def _select_treatments(candidates: list[HookTreatment], language: str) -> list[HookTreatment]:
    """Drop per-line parent spans; Arabic also drops cross-line nested substrings."""
    packed = _drop_parent_spans(candidates)
    if language == "ar":
