...
def _starts_grammatically(text: str) -> bool:
    ...
    # Relative-clause continuations need their antecedent in the same span.
    if first == "that":
        return False
...
def _ends_grammatically(text: str) -> bool:
    """Hooks must end on a sentence boundary — comma tails are clause crumbs."""
    words = [part for part in text.split() if part.strip()]
    if not words:
        return False
    last = words[-1].rstrip("\"')]}")
...
def _en_hook_spans_for_sentence(
    ...
    """Sibling clause spans only when every piece is a standalone sentence; else the full line."""
    full = _span_from_tokens(sentence_tokens, 0, len(sentence_tokens))
    full_valid = _validate_hook_unit(full, language, vocabulary)
    adjacent = [
        span
...
def _blocked_payload(
    *,
    ...
    return {
        ...
        "cites": [claim["cite"] for claim in found],
        "cards": [],
        "claims": [{"text": claim["text"], "cite": claim["cite"]} for claim in found],
        "ear": found[0]["text"] if found else "",
        "unique_texts": len({claim["text"] for claim in found}),
    }
