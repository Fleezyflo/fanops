...
def test_arabic_nested_whisper_window_farm_collapses_to_one_claim() -> None:
    """Live-shaped five overlapping Arabic lines must not count as five distinct hooks."""
    transcript = {
        "language": "ar",
        "lines": [
            {"start": 12.2, "text": "لك كفاية عزبتني عزبتني"},
            {"start": 13.0, "text": "كفاية عزبتني عزبتني"},
            {"start": 13.5, "text": "لك كفاية عزبتني"},
            {"start": 14.0, "text": "كفاية عزبتني"},
            {"start": 14.5, "text": "عزبتني عزبتني"},
        ],
    }
    ...
    assert "عذبتيني" not in result.get("source_line", "")
    assert result["cards"] == []


def test_four_english_sentences_cannot_cycle_to_twenty_files() -> None:
    """Desk must block before runner can stack-cycle four texts into twenty outputs."""
    ...
    assert plans == []
    assert len(expand_variant_slots(desk.get("cards") or [])) == 0


def test_english_mid_sentence_crumbs_never_ship() -> None:
    transcript = {
        "language": "en",
        "lines": [
            {"start": 0.0, "text": "Ross defines a true"},
            {"start": 1.0, "text": "boss by the rare ability to execute moves the real streets actually accept."},
            {"start": 5.0, "text": "So the next"},
        ],
    }
    ...
    assert texts == []
    assert not any("Ross defines a true" == t for t in texts)
    assert not any(t.lower().startswith("boss by the rare") for t in texts)
    assert not any("So the next" in t for t in texts)
