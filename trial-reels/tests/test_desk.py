...
from lib.desk import HOOKS, TARGET_VARIANTS, contract_met, is_contiguous_attested_span, write  # noqa: E402
...
def test_arabic_nested_substrings_do_not_ship_as_separate_cards() -> None:
    result = write(_load_fixture("clip_5a92132dc6de.json"))
    assert result["mode"] == "blocked"
    texts = _card_texts(result)
    assert "عذبتيني" not in texts
