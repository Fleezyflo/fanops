...
def test_english_fixture_yields_stitched_sentence_hooks() -> None:
    ...
    assert any("inside a padded recording booth creates a powerful illusion" in t for t in texts)
    assert any("Which brings us to the missing reality layer, behind-the-scenes power." in t for t in texts)
    for item in treatments:
