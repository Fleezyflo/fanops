...
def test_pipeline_requires_twenty_distinct_attested_texts() -> None:
    """Twenty mp4s with only two unique hook texts must not pass."""
    desk = write(_load_fixture("clip_twenty_hooks.json"))
    two_texts = [desk["cards"][0]["text"], desk["cards"][1]["text"]]
    payloads = [
        {
            "clip_id": f"cycle_{index}",
            "desk": desk,
            "attested_words": (two_texts[index % 2],),
        }
        for index in range(TARGET_VARIANTS)
    ]
    score = score_run(clip_payloads=payloads, stacks_landed=TARGET_VARIANTS, require_cover=False)
    assert not score.success
    assert score.distinct_verified_texts == 2
    assert "contract not met" in score.message
...
def test_pipeline_counts_twenty_distinct_when_contract_met() -> None:
    desk = write(_load_fixture("clip_twenty_hooks.json"))
    payloads = [
        {
            "clip_id": f"en_{index}",
