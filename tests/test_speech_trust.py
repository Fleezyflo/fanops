...
from fanops.transcribe import (segment_trusted, trusted_segments, window_has_trusted_speech,
                                ...
                                _trust_tier, _AVG_LOGPROB_MIN, _COMPRESSION_RATIO_MAX)
from tests.fixtures.speech_segments import (GOOD_AR, MUSIC_HALLUC, LOW_LOGPROB, LATIN_JUNK_AR,
                                              CJK_JUNK_EN, LEGACY_EN, talk_seg)
...
def test_trust_tier_matrix():
    ...
    assert _trust_tier(MUSIC_HALLUC, src_lang="en") == "full"
    assert segment_trusted(MUSIC_HALLUC, src_lang="en") is True
    assert _trust_tier(LOW_LOGPROB, src_lang="en") == "rejected"
    assert segment_trusted(LOW_LOGPROB, src_lang="en") is False
...
def test_segment_trusted_accepts_high_no_speech_when_decoder_confident():
    # no_speech_prob is a window speech-vs-music prior, not a junk-transcript detector.
    seg = _seg("لا تفكر إني بارد", no_speech_prob=0.94, avg_logprob=-0.25, compression_ratio=2.08)
    assert segment_trusted(seg, src_lang="ar") is True
...
def test_segment_trusted_recomputes_ignoring_stale_stamp():
    stamped = {**GOOD_AR, "trust_tier": "rejected", "trusted": False}
    assert segment_trusted(stamped, src_lang="ar") is True
    stale_full = {**LOW_LOGPROB, "trust_tier": "full", "trusted": True}
    assert segment_trusted(stale_full, src_lang="en") is False


def test_segment_metadata_pass_matrix():
    assert _segment_metadata_pass(GOOD_AR) is True
    assert _segment_metadata_pass(MUSIC_HALLUC) is True                     # high no_speech, decoder confident
    assert _segment_metadata_pass(LOW_LOGPROB) is False
...
def test_trusted_segments_filters_full_tier_only():
    segs = [_seg("good line", avg_logprob=-0.2, no_speech_prob=0.05, compression_ratio=1.2),
            {**LOW_LOGPROB, "text": "noise"},
            LEGACY_EN,
            {**GOOD_AR, "trust_tier": "rejected", "trusted": False}]
    ...
    assert GOOD_AR["text"] in texts                                    # stale stamp ignored; quality keys win
...
def test_window_has_trusted_speech_requires_full_tier_overlap_and_word_count():
    src = Source(id="s1", source_path="/x.mp4", state=SourceState.transcribed, language="en",
                 transcript=[_seg("they slept on me here", start=1.0, end=4.0,
                             ...
                             {**LOW_LOGPROB, "start": 10.0, "end": 12.0, "text": "noise"},
...
def test_window_has_trusted_speech_fixture_matrix():
    ...
    assert window_has_trusted_speech(hall, 0.0, 2.5) is True
    unsure = _src({**LOW_LOGPROB, "start": 0.0, "end": 2.0})
    assert window_has_trusted_speech(unsure, 0.0, 2.5) is False
...
def test_excerpt_for_window_joins_full_tier_only():
    src = Source(id="s1", source_path="/x.mp4", state=SourceState.transcribed, language="en",
                 transcript=[talk_seg("first trusted line", start=0.0, end=2.0),
                             ...
                             {**LOW_LOGPROB, "start": 1.0, "end": 3.0, "text": "noise junk"},
