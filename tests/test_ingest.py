...
def _stub_ffprobe(mocker, w=1920, h=1080, dur=12.0, *, video=True, video_pred=None):
    """OS-edge ffprobe: real has_video_stream / probe_dimensions, mocked subprocess only."""
    def run(cmd, **kw):
        path = str(cmd[-1]).lower()
        is_video = video_pred(path) if video_pred is not None else video
        if any("codec_type" in str(a) for a in cmd):
            return subprocess.CompletedProcess(cmd, 0, "video\n" if is_video else "\n", "")
        return subprocess.CompletedProcess(cmd, 0, f"{w}\n{h}\n{dur}\n", "")
    return mocker.patch("fanops.media_probe.subprocess.run", side_effect=run)

def test_sha256_stable(tmp_path):
    f = tmp_path / "a.bin"; f.write_bytes(b"hi")
    assert sha256_of(f) == sha256_of(f)

def test_ingest_drops_skips_symlinks(tmp_path):
    # ECC fix #9: a symlink in the inbox must NOT be dereferenced + ingested (it could point to a
    # file outside the data boundary). Skipped before ffprobe/copy — no source row, original stays out.
    ...
    assert (cfg.inbox / "link.mp4").is_symlink()
    assert list(cfg.sources.glob("src_*")) == []
...
def test_probe_timeout_is_per_file_fail_soft(tmp_path, mocker):
    ...
    mocker.patch("fanops.media_probe.subprocess.run", side_effect=hung)
...
def test_stage_inbox_audio_only_still_archived(tmp_path, mocker):
    ...
    mocker.patch("fanops.media_probe.subprocess.run", return_value=cp)
    staged = stage_inbox_candidates(cfg)
...
def test_catalogues_and_probes(tmp_path, mocker):
    cfg = Config(root=tmp_path); _put(cfg.inbox / "a.mp4", b"V")
    _stub_ffprobe(mocker, 1920, 1080, 12.0)
    led, _ = ingest_drops(Ledger.load(cfg), cfg)
...
def test_catalogue_stamps_created_at(tmp_path, mocker):
    ...
    cfg = Config(root=tmp_path); _put(cfg.inbox / "a.mp4", b"V")
    _stub_ffprobe(mocker, 1080, 1920, 5.0)
    led, _ = ingest_drops(Ledger.load(cfg), cfg)
    s = next(iter(led.sources.values()))
...
def test_skips_audio_only_drop(tmp_path, mocker):
    ...
    _stub_ffprobe(mocker, 1080, 1920, 5.0, video_pred=lambda p: not p.endswith(".wav"))
    led, _ = ingest_drops(Ledger.load(cfg), cfg)
    assert len(led.sources) == 1
...
def test_ingest_drains_inbox_and_does_not_rehash(tmp_path, mocker):
    ...
    # must not re-catalogue it; the original is preserved in .ingested/.
    cfg = Config(root=tmp_path); _put(cfg.inbox / "a.mp4", b"V")
    _stub_ffprobe(mocker, 1080, 1920, 5.0)
...
def test_ingest_sweeps_partial_orphans(tmp_path):
...
def test_probe_failure_catalogues_degraded_then_reprobes(tmp_path, mocker):
    ...
    def run(cmd, **kw):
        if any("codec_type" in str(a) for a in cmd):
            return subprocess.CompletedProcess(cmd, 0, "video\n", "")
        if recovered["ok"]:
            return subprocess.CompletedProcess(cmd, 0, "1080\n1920\n5.0\n", "")
...
def test_native_ingest_skips_still_photos(tmp_path):
