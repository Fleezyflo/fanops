...
def test_resolve_stamps_the_real_product_type(tmp_path, monkeypatch):
    ...
    reconcile.resolve_media_ids(led, cfg, get=_media_get([page]))
    assert led.posts["p1"].media_id == "M1"
    assert led.posts["p1"].post_type == "FEED"    # the real type, stamped alongside media_id
...
def test_resolve_skips_when_no_media_id_field_on_model(tmp_path, monkeypatch):
    ...
    # no wasted call). (A media_id-bearing row with post_type=None IS re-targeted — see the M2 test below.)
    cfg = _cfg(tmp_path, monkeypatch)
    p = _post("p1", "https://www.instagram.com/reel/AAA/")
    p = p.model_copy(update={"media_id": "PRESET", "post_type": "REELS"})
    led = _led(cfg, [p])
...
def test_resolve_backstamps_product_type_on_a_media_id_bearing_row(tmp_path, monkeypatch):
    ...
    # product_type was carried) has media_id set but post_type=None. The pre-M2 target filter was
    ...
    cfg = _cfg(tmp_path, monkeypatch)
    p = _post("p1", "https://www.instagram.com/reel/AAA/")
    p = p.model_copy(update={"media_id": "M1", "post_type": None})   # resolved id, type not yet carried
    led = _led(cfg, [p])
    page = _Resp(200, {"data": [{"id": "M1", "permalink": "https://www.instagram.com/reel/AAA/",
                                 "media_product_type": "REELS"}]})
    reconcile.resolve_media_ids(led, cfg, get=_media_get([page]))
    assert led.posts["p1"].media_id == "M1"          # media_id preserved (not re-fabricated)
    assert led.posts["p1"].post_type == "REELS"   # the real type back-stamped -> request no longer empty
