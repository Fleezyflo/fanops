...
from fanops.personas import (Persona, compose_breakdown, resolved_cut_spec, casting_directive,
                             hook_directive, caption_directive, derive_cut_spec)
from fanops.signals import filter_peaks_by_intensity
import fanops.persona_levers as pl


# Live quarantine: EMPTY (M3f). Freeze the empty set as a literal ratchet — `set() <= set()` is not a pin.
_KNOWN_INCOHERENT = set()
...
# settings; cut_policy is punchlines vs storytelling so derive_cut_spec framing actually moves.
_MUTATIONS = {
    "voice": ("a devoted fan", "a blunt critic"),
    "content_focus": ("clip the punchlines", "clip the loudest hype"),
    "cut_policy": (["punchlines"], ["storytelling"]),
    "intensity": ("high", "low"),
...
_PEAKS = [{"t": float(i), "kind": "scene_cut", "score": i / 10.0} for i in range(1, 10)]


def _output(cfg, p):
    """The full compiled fingerprint a persona produces — the bytes that actually reach the pipeline."""
    d = compose_breakdown(cfg, p)
    _, framing = derive_cut_spec(p)
    filtered = filter_peaks_by_intensity(_PEAKS, getattr(p, "intensity", None))
    return (d["casting"]["text"], d["hook"]["text"], d["caption"]["text"],
            d["cut"]["framing"], framing, tuple(d["tags"]["lead"]),
            tuple(d["tags"]["terms"]),
            tuple((pk["t"], pk.get("score")) for pk in filtered))


def test_quarantine_is_empty_literal():
    assert _KNOWN_INCOHERENT == set()
...
def test_each_lever_mutation_changes_the_compiled_output(tmp_path):
    # WIRED: a lever that claims to differentiate MUST move compiled output (not an input echo).
    ...
    punch = Persona(id="p", cut_policy=["punchlines"])
    story = Persona(id="p", cut_policy=["storytelling"])
    assert derive_cut_spec(punch)[1] == "center"
    assert derive_cut_spec(story)[1] == "top"
    assert derive_cut_spec(punch) != derive_cut_spec(story)
    assert filter_peaks_by_intensity(_PEAKS, "high") != filter_peaks_by_intensity(_PEAKS, "low")
