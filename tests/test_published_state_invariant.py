...
# ───────────────────────────────────────────────────────────────────────────
# Doctor migration — heal any pre-existing ghost rows in a production ledger
# (the R1 invariant would otherwise refuse to LOAD a ledger that already has them)
# ───────────────────────────────────────────────────────────────────────────

def test_doctor_fix_ghosts_backfills_dryrun_url_when_sidecar_exists(tmp_path):
    """RED: a pre-existing ghost row (state=published, public_url='') with a matching
    05_scheduled/<id>.json sidecar gets healed to public_url='dryrun://<id>'. This proves the
    ghost row came from the dryrun path (the sidecar is the smoking gun)."""
    from fanops.cli import cmd_doctor_fix_ghosts
    ...
    # Pre-create a sidecar to prove dryrun-origin
    cfg.scheduled.mkdir(parents=True, exist_ok=True)
    (cfg.scheduled / "post_ghost1.json").write_text('{"text": "c"}')
    # Pre-create a ghost row directly in the ledger JSON to bypass the post-R1 Pydantic invariant
    # (this simulates a pre-R1 ledger being loaded by a post-R1 doctor)
    import json
    ...
    ledger_path.write_text(json.dumps({
        ...
        "posts": [{
            "id": "post_ghost1", "parent_id": "c_t", "account": "@a", "account_id": "ig_a",
            "platform": "instagram", "caption": "c", "state": "published",
            "public_url": "", "published_at": "2026-06-29T12:10:01Z",
        }],
        "renders": [], "stitches": [], "batches": [],
    }))

    import argparse
    args = argparse.Namespace()
    rc = cmd_doctor_fix_ghosts(cfg, args)
    assert rc == 0
    # The cli reads the ledger BYPASSING the invariant (raw JSON read + manual repair + write)
    healed = json.loads(ledger_path.read_text())
    posts = {p["id"]: p for p in healed["posts"]}
    assert posts["post_ghost1"]["public_url"] == "dryrun://post_ghost1", (
        f"ghost row with sidecar MUST be healed to dryrun://; got {posts['post_ghost1']!r}")


def test_doctor_fix_ghosts_parks_unknown_origin_in_needs_reconcile(tmp_path):
    """RED: a ghost row WITHOUT a sidecar (origin unknown) must NOT be assumed dryrun. Park it
    in needs_reconcile so the reconciler can investigate. Fail-closed."""
    from fanops.cli import cmd_doctor_fix_ghosts
    ...
    ledger_path.write_text(json.dumps({
        "sources": [], "moments": [], "clips": [],
        "posts": [{
            "id": "post_ghost2", "parent_id": "c_t", "account": "@a", "account_id": "ig_a",
            "platform": "instagram", "caption": "c", "state": "published",
            "public_url": "", "published_at": "2026-06-29T12:10:01Z",
        }],
        "renders": [], "stitches": [], "batches": [],
    ...
    healed = json.loads(ledger_path.read_text())
    posts = {p["id"]: p for p in healed["posts"]}
    assert posts["post_ghost2"]["state"] == "needs_reconcile", (
        f"unknown-origin ghost row MUST be parked in needs_reconcile, not auto-labeled dryrun; "
        f"got state={posts['post_ghost2']['state']!r}")
