...
def test_schedule_rows_batch_filter_and_label(tmp_path):
    ...
    led.add_post(Post(id="p_u", parent_id="clip_0", account="a0", account_id="0",
                      platform=Platform.instagram, caption="c", state=PostState.queued, scheduled_time=FAR, public_url="https://www.instagram.com/p/p_u/")); led.save()
    led = Ledger.load(cfg)
...
def test_posted_library_and_rollup_exclude_dryrun_url(tmp_path):
    # leftover published+dryrun:// cannot rest — must not increment Posted / rollup / /posted.
    from pydantic import ValidationError
    cfg = Config(root=tmp_path); _accounts(cfg)
    _seed(cfg, 1, state=PostState.published, batch_id="bx", batch_name="Drop")
    with pytest.raises(ValidationError):
        Post(id="p_dry", parent_id="clip_0", account="a0", account_id="0",
