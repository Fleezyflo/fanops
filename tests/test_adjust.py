...
def test_amplify_then_ingest_then_render_produces_new_clip(tmp_path, monkeypatch):
    ...
    response_path(cfg, "moments", "src_1").write_text(MomentDecision(
        ...
        picks=[MomentPick(start=16.0, end=30.0, reason="second wave like the first")]).model_dump_json())
    led = ingest_moments(led, cfg, "src_1")
    new = [m for m in led.moments_of("src_1") if m.content_token == "16.00-30.00"]
    assert len(new) == 1
...
def test_amplify_preserves_winners_published_lineage(tmp_path, monkeypatch):
    ...
    response_path(cfg, "moments", "s1").write_text(MomentDecision(
        ...
        picks=[MomentPick(start=16.0, end=30.0, reason="second wave")]).model_dump_json())
    ...
    assert any(m.content_token == "16.00-30.00" for m in led.moments_of("s1"))
