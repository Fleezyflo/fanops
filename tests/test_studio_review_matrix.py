...
def _seed(cfg):
    ...
    led.add_post(Post(id="p_m1_a", parent_id="c1", account="@a", account_id="1", platform=Platform.instagram, caption="A", state=PostState.awaiting_approval, render_id="r_a"))
    led.add_post(Post(id="p_m1_b", parent_id="c1", account="@b", account_id="2", platform=Platform.instagram, caption="B", state=PostState.awaiting_approval))
    # m2: @a IG ORIGINAL + a REPOST (later created_at, epoch-suffixed) on the SAME channel → multiplicity 2, lead=repost.
    led.add_post(Post(id="p_m2_a", parent_id="c2", account="@a", account_id="1", platform=Platform.instagram, caption="A2", state=PostState.awaiting_approval, created_at=_z(NOW)))
    ...
    led.add_post(Post(id="p_m2_b_ig", parent_id="c2", account="@b", account_id="2", platform=Platform.instagram, caption="Big", state=PostState.awaiting_approval, render_id=None))
    led.add_post(Post(id="p_m2_b_tt", parent_id="c2", account="@b", account_id="2", platform=Platform.tiktok, caption="Btt", state=PostState.awaiting_approval))
    led.save()
