...
def test_policy_maps_to_length():
    # MOL-523: cut_policy now drives the cut.
    assert derive_cut_spec(_p(cut_policy=["storytelling"]))[0] == "long"
    assert derive_cut_spec(_p(cut_policy=["punchlines"]))[0] == "short"
    assert derive_cut_spec(_p(cut_policy=["emotional"]))[0] == "medium"
...
def test_multi_policy_is_deterministic_longer_bias_first():
    assert derive_cut_spec(_p(cut_policy=["punchlines", "storytelling"]))[0] == "long"
    assert derive_cut_spec(_p(cut_policy=["storytelling", "punchlines"]))[0] == "long"
...
def test_account_carrier_pin_beats_derived():
    ...
    assert resolved_cut_spec(acc) == ("medium", "top")

def test_persona_can_no_longer_pin_only_derives():
    p = _p(cut_policy=["punchlines", "hype"], clip_profile="medium", framing="top")
    assert resolved_cut_spec(p) == ("short", "center")

def test_derived_when_unpinned():
    assert resolved_cut_spec(_p(cut_policy=["punchlines", "hype"])) == ("short", "center")
...
def test_compose_breakdown_cut_source_three_way(tmp_path):
    ...
    assert "28-45s" in compose_breakdown(cfg, _p(cut_policy=["storytelling"]))["cut"]["band"]
...
def test_voice_match_hydrates_without_persona_id(tmp_path):
    ...
    acc = next(a for a in Accounts.load(cfg).accounts if a.handle == "a")
    assert acc.persona_id is None and acc.clip_profile == "long" and acc.framing == "top"
