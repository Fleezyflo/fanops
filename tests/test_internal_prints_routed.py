...
_CLI_PRINT_COUNT = 176  # CPDP-WP3: studio launch dropped for-line prints over health.ensure_up log list (bare postiz_lifecycle.ensure_up); prior 177 # +4 MOL-965 WP1 severity honesty: cmd_health surfaces FAIL/UNKNOWN/WARN check lines (+3); cmd_autopilot prints readiness when deps fail severity without a failed check (+1); prior 173 (+1 studio --app refuse/open path; prior 172 (+3 ::1 foreign-listener REFUSED lines on studio start; prior 169 (stderr): an operator who types the
# bare verb must be TOLD why it refused and which flag to use instead — a user-facing refusal exactly like
# cmd_reframe's --dry-run guard, NOT an internal diagnostic. (`posts census-retired` adds none: its whole
# report lives in fanops/stranded_posts.py.)  # +B11 cmd_doctor dep lines (+2); +studio --install/--uninstall + resident-loaded guard + MOL-223 wipe verb; +3 cmd_up (user-facing bring-up report — like cmd_daemon/cmd_autopilot); +2 root-divergence WARN + daemon-status two-roots line (user-facing operator diagnostics); +11 cmd_reframe (user-facing read-only report: the --dry-run refusal, the JSON manifest, and the per-class / root-cause / protected-root / go-no-go lines — an operator report exactly like cmd_doctor's, NOT an internal diagnostic); +7 _reframe_mutation (the MUTATION verbs' operator report: the exactly-one-verb refusal, --status/--rollback/--cleanup/--apply JSON, the missing/partial-manifest refusals, and the NOT-CLEAN alarm — an operator MUST see what a migration did to their corpus); +3 cmd_canary (operator-facing report for the isolated canary verbs — the unknown-subcommand refusal + the failure line to stderr + the JSON result to stdout — exactly like cmd_reframe/cmd_doctor, NOT an internal diagnostic)
