...
_CLI_PRINT_COUNT = 169  # +1 the `fanops studio` unmanaged-foreground REFUSAL (stderr): an operator who types the
# bare verb must be TOLD why it refused and which flag to use instead — a user-facing refusal exactly like
# cmd_reframe's --dry-run guard, NOT an internal diagnostic. (`posts reconcile-retired` adds none: its whole
