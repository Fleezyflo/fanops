...
def _seed(cfg):
    ...
    with Ledger.transaction(cfg) as led:
        ...
        led.add_post(Post(id="p_m1_a", parent_id="c1", account="@a", account_id="1", platform=Platform.instagram, caption="A", state=PostState.awaiting_approval))
        ...
        led.add_post(Post(id="p_m2_bi", parent_id="c2", account="@b", account_id="2", platform=Platform.instagram, caption="Bi", state=PostState.awaiting_approval))
        led.add_post(Post(id="p_m2_bt", parent_id="c2", account="@b", account_id="2", platform=Platform.tiktok, caption="Bt", state=PostState.awaiting_approval))
        # src0 has its own awaiting post (proves the picker + that default focus is src1, not src0)
        led.add_post(Post(id="p_m0_a", parent_id="c0", account="@a", account_id="1", platform=Platform.instagram, caption="Old", state=PostState.awaiting_approval))
