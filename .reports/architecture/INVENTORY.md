...
| `FIND-009` | **`Settings` is never constructed by `Config`.** `Config.__init__` sets paths only; Config properties read the env directly, uncached, per access. Dual plane: Config = live getenv façade; Settings = doctor / `fanops config` strict evaluation of the same declarations. **No runtime precedence conflict exists, because there is no runtime handoff.** | **doc drift (resolved in source)** |
