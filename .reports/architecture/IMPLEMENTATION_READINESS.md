...
| Attack | Result |
|---|---|
| **Hidden coupling** | 🔴 **FOUND ONE.** `cli.py`'s `_CLI_PRINT_COUNT = 168` is an **exact-equality** assertion, and **three slices touch `cli.py`.** Two landing with a bumped constant break each other. **Cycle 4 did not name it.** → **Repaired:** *no slice may change the print count* (`GB-6`/`IR-4`). |
