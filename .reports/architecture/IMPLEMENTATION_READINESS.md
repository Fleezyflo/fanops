...
| Attack | Result |
|---|---|
| **Hidden coupling** | 🔴 **FOUND ONE.** `cli.py`'s `_CLI_PRINT_COUNT` constant — **147** at this report's Cycle-6 freeze — is an **exact-equality** assertion, and **three slices touch `cli.py`.** Two landing with a bumped constant break each other. **Cycle 4 did not name it.** → **Repaired:** *no slice may change the print count* (`GB-6`/`IR-4`). *(Historical record: the number above is the Cycle-6 value and is deliberately NOT rewritten when `cli.py` changes. The current enforced value lives only in `tests/test_internal_prints_routed.py`.)* |
